...
2. [`presentation/GUI_Agent记忆机制探索_夏令营最终汇报_v7_诊断验证版.pdf`](presentation/GUI_Agent记忆机制探索_夏令营最终汇报_v7_诊断验证版.pdf)：加入 R2 与 MobileUse Hard 数值/复杂度语境、官方 Hard 任务构成、旧实验断点证据表和框架验证边界后的 13 页、15 分钟正式汇报。
...
3. 正面讲清 A0→A1→R2：从引入结构化任务账本，到压缩为最小 verified/pending 状态；同时交代每一步为何应当有效、实际改善了什么、付出了什么成本。
4. 把 R2 的 6/19 作为完整系统主结果：在不同实验协议的明确限定下，与 MobileUse 最强 Hard 消融的 31.6% 作数值坐标，并强调 R2 零额外模型调用的复杂度差异。
5. 用官方任务标签说明 19 个 Hard 类混合记忆、多应用、复杂 UI、搜索、数据录入与计算等需求，因此不事后挑选“记忆子集”，而用完整分母评价轻量干预。
6. 将后续干预按“未激活、激活但无行为分叉、局部修复不足、成本改善”审计；用 R5-R12 表证明相同 0 分包含不同信息断点。
7. 先讲 R15 Browser 的“成功但组件无信用”，再用七题 4/7 限制作用域，避免单例外推。
8. 与 MobileUse、MemGUI-Bench 比较主要分析尺度，再把本项目定位为 fact × stage × trajectory 的更细信用分配。
9. 明确 L0-L6 当前只获得描述性映射与回顾性区分支持；前瞻预测必须通过盲法标注和预注册匹配干预验证。
