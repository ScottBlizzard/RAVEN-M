...
本项目最可靠的完整系统结果仍是 R2 的 6/19。综合纵向、横向、R15 与 A4-v2 证据，更统一的解释是：记忆可以保存已有能力、复用已有成功经验，但在本设置中未能单独补齐缺失的路线规划、视觉定位、无进展恢复和完成核验能力。R15 暴露“成功与组件因果信用可以错位”，A4-v2 证明经验复用还可能在 donor coverage 前置条件上被阻断，因此分析框架从组件级比较推进为 P0+L0-L6：先审方法是否有机会成立，再追踪任务事实与行为结果。

## 建议阅读顺序

1. [`PRO_FINAL_REPORT_RETHINK.md`](PRO_FINAL_REPORT_RETHINK.md)：Pro 对全部证据、故事线、逐页方案与答辩边界的完整重审。
2. [`presentation/GUI_Agent记忆机制探索_夏令营最终汇报_v10_能力边界版.pdf`](presentation/GUI_Agent记忆机制探索_夏令营最终汇报_v10_能力边界版.pdf)：12 页、15 分钟正式汇报；先建立 A0→A1→R2 的纵向证据和横向对照，再由 R15 与 donor coverage 推出 P0+L0-L6，最后将全部结果统一为“保存已有能力—复用已有经验—不能替代能力生成”的边界，并给出 expert donor × memory ON/OFF 的决定性验证。
...
4. 展示同时开展的横向机制探索，先区分“保存状态、复用经验、补齐能力”三个不同问题，再用 A4-v2 的 24 个 donor slot 证明经验复用还可能受 donor coverage 前置条件限制。
...
10. 将全部证据统一为一个能力边界：R2 保存已有任务状态；AWM 类方法复用已有成功经验；路线、感知、恢复与核验能力若尚未形成，本设置中的纯记忆改造未能单独补齐。
11. 用 expert donor × memory ON/OFF 的控制实验区分“缺少可复用经验”和“执行器能力不足”，把解释转化为可证伪预测。
