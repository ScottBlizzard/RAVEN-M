...
2. [`presentation/GUI_Agent记忆机制探索_夏令营最终汇报_v6_纵向闭环版.pdf`](presentation/GUI_Agent记忆机制探索_夏令营最终汇报_v6_纵向闭环版.pdf)：补全 A0→A1→R2 机制演进与负结果解释后的 13 页、15 分钟正式汇报。
...
4. 把 R2 的 6/19 作为完整系统主结果，再拆解其余 13 题为长程推进失败与错误完成判断，而不是只报总分。
5. 将后续干预按“未激活、激活但无行为分叉、局部修复不足、路径不可归因”审计，解释为什么有针对性的尝试仍未提升总分。
6. 先讲 R15 Browser 的“成功但组件无信用”，再用七题 4/7 限制作用域，避免单例外推。
7. 从 Browser、Calendar、Recipe、Osm 的不同瓶颈推出 L0–L6 信息机会链，而不是先给框架再挑案例。
8. 与相关工作只比较研究对象与归因尺度，并以 Claim–Evidence–Decisive Test 明确现有结论和下一步判别实验。
