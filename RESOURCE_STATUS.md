...
| P1 核心论文 PDF | 8 | 8 | 完整 |
| P2 扩展论文条目 | 14 | 14 | 已建索引，按需下载 |
| 全部论文元数据 | 37 | 37 | 已登记 |
| 核心 BibTeX | 20 | 20 | 无重复 citation key |
| 第三方代码仓库 | 10 | 11 | 1 个网络原因待补 |
| 官方网页快照 | 6 | 6 | 完整 |
...
- 10 个第三方仓库的本地浅克隆及精确 HEAD commit 锁定记录。
- 下载状态、代码来源、检索过程、BibTeX、SHA-256 校验和及完整性审计脚本。

## 唯一未闭合项

`MobileAgent` 官方仓库已由论文与项目来源交叉确认，但在当前网络下两次浅克隆均长时间停滞，因此没有保留不完整目录。它被明确标为 `large_pending`：

- 官方地址：`https://github.com/X-PLUG/MobileAgent`
- 本地登记：`03_code/manifests/repositories.csv`
- 相关论文 PDF 已在 P0/P1 资料层中准备好

网络恢复后可只补这一项，不需要重新下载其余仓库。
