# SYS-TRRC-v2 — R2 one-shot triggered recovery preregistration

Date: 2026-08-15 (Asia/Hong_Kong)

Status: prospective V2 scientific contract. Live generation remains forbidden
until the V2 implementation, replay, source freeze, and zero-generation
preflight all pass under one clean committed identity.

## 1. Identity and claim boundary

- Protocol ID: `SYS_TRRC_R2_ONE_SHOT_RECOVERY_PREREG_V2`
- System ID: `sys_trrc_r2_one_shot_triggered_recovery_v2`
- System family: `SYS-TRRC-v2`
- Frozen parent mechanism: `a1r2_compact_verified_pending_v1`
- Frozen V1 terminal-evidence commit:
  `ad522db47a40421a08f64d1896d14b743149add1`
- Task seed: `20260806`
...
### 1.1 V2's only mechanism-level delta

V1 produced one Full auxiliary response on `ExpenseDeleteMultiple2`, but the
otherwise well-formed three fields were separated by blank lines. The frozen
V1 parser rejected it as `aux_schema`; no advice reached the executor and V1
stopped at its first Full preservation task. The exact V1 response and hashes
are frozen in `evidence/sys_trrc/SYS_TRRC_FULL_L1_EXPENSE_EPISODE_2026-08-15.json`.

V2 changes only the deterministic delivery parser: lines containing whitespace
only are discarded before validating the response. The remaining nonblank
lines must still be exactly three lines, in the exact order `ASSESSMENT`,
`RECOMMENDATION`, `VISIBLE_CHECK`, with each key appearing exactly once and no
additional prose or multiline continuation. All forbidden-content, field,
render, token, latency, detector, R2, prompt, wrapper, and call-budget rules are
unchanged. The V1 response is a frozen regression fixture, not live tuning data.
...
- Arm ID: `SYS-TRRC-V2-R2-DETECTOR`
...
The V2 detector replay is bound to:
...
- report `content_sha256`:
  `23dcd8979a67584ec92c5206f5cea4d20fd52498b6da798bddb87e903fdc089f`;
- evidence-file SHA-256:
  `371743805736ebee4d88a303951afe95cc38816d2a32dab733914aadd2c3e940`;
...
The separately frozen V1 report remains the immutable provenance source for
the already-materialized eight PNG/token-projection inputs. V2 recomputation
must match its ordered trigger manifest and behavior counts exactly; V2 audit
record hashes differ only because the system/audit schema identity is V2.
...
The exact encoding, field bounds, parser, tokenizer projection, and prompt
hashes must be frozen before live generation. Blank-only separator lines are
ignored exactly as specified in section 1.1; no other whitespace or schema
recovery is allowed. There is no free-form fallback and no second call.
...
### Gate L1: Full-first preservation, then controls

To avoid paying for controls when the primary system cannot preserve its first
task, run `ExpenseDeleteMultiple2` once in the frozen order:

1. Full
2. Base, only if Full succeeded
3. Detector-only, only if Full succeeded
4. Generic, only if Full succeeded

All valid outcomes are retained. Full failure is immediately terminal and the
three controls are not run. Full success without a trigger is a valid silent
preservation result and does not count as component evidence. Control-arm
failures do not authorize a Full rerun.

### Gate L2: R2-six preservation

Full then Generic run, without repeating completed episodes, in this order:
...
After Full reaches 6/6, run Full first, then Base, Detector-only, and Generic
once on the frozen activation task `BrowserMultiply`; a qualifying event on an
...
The exact campaign invocation order is frozen as:

```text
F1 -> B1 -> D1 -> G1 -> F2 -> G2 -> F3 -> B3 -> D3 -> G3 -> F4 -> G4
```

Any Full terminal failure prevents every later invocation. Control failures are
retained but do not authorize a Full rerun.
