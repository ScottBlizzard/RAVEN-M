"""A10 Evidence-Calibrated Obligation--Branch Frontier memory.
...
class A10VisibleInputError(RuntimeError):
    """The controller did not provide a legal model-visible RGB frame."""


class A10IntegrityError(RuntimeError):
...
def _visible_rgb(snapshot: dict[str, Any]) -> np.ndarray:
    ...
    if (
        ...
        raise A10VisibleInputError("A10 requires integer model-visible RGB pixels")
    if pixels.size and (int(pixels.min()) < 0 or int(pixels.max()) > 255):
        raise A10VisibleInputError("A10 RGB values must be within [0,255]")
...
def extract_goal_anchors(goal: str, max_anchors: int = 8) -> list[GoalAnchor]:
    ...
    for weight, offset, kind, literal in candidates:
        ...
        anchors.append(GoalAnchor(
            anchor_id=f"a10a_{sha256(normalized.encode('utf-8')).hexdigest()[:12]}",
...
def _canonical_action(action: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(action, dict):
        raise A10IntegrityError("canonical action must be a mapping")
    action_type = str(action.get("type") or "")
    allowed = {"tap", "long_press", "swipe", "type_text", "press_back", "press_home", "press_enter", "press_recents", "wait", "answer"}
    if action_type not in allowed:
        raise A10IntegrityError(f"unsupported canonical action: {action_type!r}")
    return dict(action)


def _coord(value: Any) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)) or not math.isfinite(float(value)):
        raise A10IntegrityError("canonical coordinate must be finite")
    result = float(value)
    if not 0.0 <= result <= 1.0:
        raise A10IntegrityError("canonical coordinate outside [0,1]")
    return result
...
_ANCHOR_WEIGHTS = {
    ...
    "ROUTE_RETURN": -0.25, "REVERSAL_OR_FAILURE_PROSE": -0.45,
    "LATER_REOPEN_ATTEMPT": -0.30,
}


class EvidenceCalibratedObligationBranchFrontierMemory:
    ...
    def __init__(self, *, max_anchors: int = 8, max_anchor_events: int = 6,
        ...
        self._no_anchor_phase_watch: EscapeWatch | None = None
    ...
    def _initialize_goal_once(self, goal: str) -> None:
        ...
        if not self.goal_sha256:
            ...
            raise A10IntegrityError("goal changed within an episode")
    ...
    def _refresh_anchors(self, step: int) -> None:
        for anchor in self.anchors:
            ...
            if anchor.ever_supported and strong_negative and anchor.contradiction_count > 0:
    ...
    def _branch(self, frontier: FrontierRecord, family: tuple[Any, ...], intent: str, mask: int, summary: str, step: int) -> tuple[BranchRecord, bool]:
        key = _json_sha([family, intent, mask])
    ...
    def _evict_branches(self, frontier: FrontierRecord, step: int) -> None:
        while len(frontier.branches) > self.max_branches_per_frontier:
            def utility(item: tuple[str, BranchRecord]) -> tuple[float, int, str]:
                ...
                u = 2 * bad + escape + 0.5 * bool(branch.target_anchor_mask & self._open_mask()) + math.exp(-max(0, step - branch.last_step) / 8)
    ...
    def _resolve_routes(self, step: int, descriptor: VisualDescriptor) -> list[dict[str, Any]]:
        ...
        for route in self.pending_routes:
            ...
            if returned and 1 <= age <= 4:
                outcome = "RETURNED"
                if receipt:
                    receipt.resolved_outcome, receipt.resolve_step, receipt.route_length = outcome, step, age
                if branch:
            ...
            if returned and 5 <= age <= 8:
                ...
                continue
            if age >= 4 and not route.durable_emitted:
                route.durable_emitted = True
                if receipt:
                    receipt.resolved_outcome, receipt.resolve_step, receipt.route_length = "DURABLE_DEPARTURE", step, age
                if branch:
    ...
    def _make_trigger(self, kind: str, step: int, frontier: FrontierRecord, descriptor: VisualDescriptor, strength: float, route_strength: float, gain: float, payload: dict[str, Any]) -> TriggerCandidate:
        ...
        expires = step + (6 if kind == "PARTIAL_OBLIGATION_ESCAPE" else 8)
        return TriggerCandidate(f"a10t_{kind[:2].lower()}_{step}_{signature[:10]}", kind, step, expires, self.phase_id, self._open_mask(), frontier.frontier_id, descriptor, min(1.0, max(0.0, strength)), min(1.0, max(0.0, route_strength)), max(0.0, gain), signature, bounded_payload)
    ...
    def _enqueue(self, trigger: TriggerCandidate) -> bool:
        ...
        while len(self.trigger_candidates) > self.max_trigger_candidates:
            bonus = {"PARTIAL_OBLIGATION_ESCAPE": .25, "CLOSED_ROUTE_WITHOUT_ADVANCE": .20, "BAD_BRANCH_REPEAT": .15, "VALUE_REENTRY_AFTER_BAD_OUTCOME": .10, "FRONTIER_COLLAPSE": .05}
    ...
    def _unresolved_ratio(self) -> float:
        if not self.anchors:
            return 1.0
        total = sum(a.specificity_weight for a in self.anchors)
        return sum(a.specificity_weight for a in self.anchors if a.status != "LOCALLY_SUPPORTED") / total
    ...
    def _candidate_distance(self, trigger: TriggerCandidate, descriptor: VisualDescriptor) -> tuple[float, bool]:
        ...
        if trigger.expected_descriptor.exact_sha256 == descriptor.exact_sha256:
            return 0.0, True
        return dv, dl <= .06 and de <= .12 and dv <= .055
    ...
    def _score(self, trigger: TriggerCandidate, descriptor: VisualDescriptor, step: int, frontier: FrontierRecord | None) -> tuple[float, dict[str, float]]:
        ...
        m = 1.0 if distance == 0 else max(0.0, 1.0 - distance / .055) if matches else 0.0
        e = trigger.evidence_strength
        o = self._unresolved_ratio()
        g = 1.0 - min(1.0, trigger.anchor_gain / .15)
        visits = len([x for x in (frontier.recent_visit_steps if frontier else []) if step - x <= 7])
        v = 1.0 if trigger.kind == "PARTIAL_OBLIGATION_ESCAPE" else min(1.0, max(0, visits - 1) / 2.0)
        r = min(1.0, trigger.route_return_strength)
        f = math.exp(-max(0, step - trigger.created_step) / 8)
        score = m * (.30 * e + .20 * o + .20 * g + .15 * v + .10 * r + .05 * f)
        return score, {"M": m, "E": e, "O": o, "G": g, "V": v, "R": r, "F": f}

    def _render(self, trigger: TriggerCandidate) -> str:
        unresolved = [a for a in self.anchors if a.status != "LOCALLY_SUPPORTED"]
        label_limit = 20 if len(unresolved) > 1 else 24
        labels = [f'"{_compact(a.literal, label_limit)}"' for a in unresolved[:2]]
        open_text = ", ".join(labels) if labels else "task completion is not established"
        if len(unresolved) > 2:
            open_text += f" (+{len(unresolved) - 2} more)"
        if len(open_text) > 56:
            labels = [f'"{_compact(a.literal, 16)}"' for a in unresolved[:2]]
            open_text = ", ".join(labels)
            if len(unresolved) > 2:
                open_text += f" (+{len(unresolved) - 2} more)"
        supported = sorted((a for a in self.anchors if a.status == "LOCALLY_SUPPORTED"), key=lambda a: a.last_evidence_step or -1, reverse=True)
        supported_text = f'"{_compact(supported[0].literal, 24)}"' if supported else "none"
        ...
        if trigger.kind == "PARTIAL_OBLIGATION_ESCAPE":
        ...
        elif trigger.kind == "CLOSED_ROUTE_WITHOUT_ADVANCE":
            evidence = f'{payload.get("branch", "this route")} left this screen and returned in {payload.get("route_length", 4)} actions without open-item gain'
            ...
            evidence = f'this decision screen appeared {payload.get("visits", 3)}x in 7 steps with no durable open-item advance'
        evidence = _compact(evidence, 90)
        ...
        rendered = (
            ...
            f"Open: {open_text}. Supported locally: {_compact(supported_text, 40)}. Evidence: {evidence}.\n"
            + suffix
        )
        if len(rendered) > self.max_chars or len(rendered.encode("utf-8")) > self.max_utf8_bytes:
            ...
            rendered = (
                "A10 frontier; past visible evidence only, current screen wins.\n"
                f"Open: {_compact(open_text, 40)}. Supported locally: {_compact(supported_text, 24)}. Evidence: {evidence}.\n"
                + suffix
            )
        if len(rendered) > self.max_chars or len(rendered.encode("utf-8")) > self.max_utf8_bytes:
            raise A10IntegrityError("A10 fixed renderer exceeded its frozen budget")
        return rendered
        ...
        for event in self.read_events:
            ...
            delta = max(
                (
                    ...
                    for index, anchor in enumerate(self.anchors)
                    if index < len(previous) and read_open_mask & (1 << index)
                ),
                default=0.0,
            )
    ...
    def read(self, context: dict[str, Any] | None = None) -> tuple[str, dict[str, Any]]:
        ...
        if self.nonempty_read_count >= self.max_nonempty_reads:
            ...
            ranked = []
            priority = {"PARTIAL_OBLIGATION_ESCAPE": 5, "CLOSED_ROUTE_WITHOUT_ADVANCE": 4, "BAD_BRANCH_REPEAT": 3, "VALUE_REENTRY_AFTER_BAD_OUTCOME": 2, "FRONTIER_COLLAPSE": 1}
            for trigger in self.trigger_candidates:
                if trigger.phase_id != self.phase_id or trigger.open_anchor_mask != open_mask or trigger.evidence_signature in self.delivered_signatures or step > trigger.expires_step:
                    continue
                distance, matches = self._candidate_distance(trigger, descriptor)
                if not matches:
                    continue
            ...
            if ranked:
                ...
                event = {"read_id": f"a10r_{step}_{selected.evidence_signature[:8]}", "step": step, "trigger_id": selected.trigger_id, "trigger_kind": selected.kind, "frontier_id": selected.query_frontier_id, "phase_id": self.phase_id, "open_anchor_mask": open_mask, "score": score, "score_components": components, "visual_distance": distance, "evidence_signature": selected.evidence_signature, "rendered_sha256": sha256(rendered.encode()).hexdigest(), "rendered_chars": len(rendered), "rendered_utf8_bytes": len(rendered.encode()), "retrieved_anchor_ids": [a.anchor_id for a in self.anchors if selected.open_anchor_mask & (1 << self.anchors.index(a))][:3], "retrieved_branch_ids": [str(selected.evidence_payload.get("branch_id") or "")][:1], "next_action_branch_id": None, "next_action_was_novel": None, "escaped_frontier_within_3": None, "returned_within_4": None, "open_anchor_confidence_delta_within_4": None}
    ...
    def observe_step(self, **kwargs: Any) -> dict[str, Any]:
        ...
        mask = target_anchor_mask(summary, action, self.anchors)
        family = canonical_action_family(action)
        branch, branch_created = self._branch(source, family, intent, mask, summary, step)
        ...
        for pending in self.pending_routes:
            pending.max_open_anchor_gain = max(
                ...
                max(
                    (
                        anchor.confidence - pending.base_confidences[index]
                        for index, anchor in enumerate(self.anchors)
                        if pending.open_anchor_mask & (1 << index)
                    ),
                    default=0.0,
                ),
            )
            ...
            if (
                ...
                and mask != pending.target_anchor_mask
                and (mask or pending.target_anchor_mask)
            ):
        ...
        if not self.anchors and intent == "COMMIT" and outcome == "LOCAL_VISIBLE_CHANGE":
            self._no_anchor_phase_watch = EscapeWatch(
                source_step=step,
        ...
        gain = max(
            (
                anchor.confidence - source.anchor_confidence_at_first_visit[index]
                for index, anchor in enumerate(self.anchors)
                if source.open_anchor_mask & (1 << index)
            ),
            default=0.0,
        )
        if not phase_switch:
            if (n >= 1.8 or (n >= .9 and r >= .9)) and gain < .15 and not self._retry_exempt(branch, action, before_desc, step, gain):
                trigger = self._make_trigger("BAD_BRANCH_REPEAT", step, source, before_desc, bad, r, gain, {"branch": branch.label, "branch_id": branch.branch_id, "count": max(2, branch.raw_no_progress_count + branch.raw_return_count)})
                if self._enqueue(trigger): enqueued.append(trigger.trigger_id)
            for resolution in route_resolutions:
                if (
                    resolution["outcome"] == "RETURNED"
                    and resolution["route_length"] <= 4
                    and resolution["phase_open_unchanged"]
                    and resolution["target_anchor_unchanged"]
                    and resolution["open_anchor_mask"] == new_mask
                    and resolution["route_anchor_gain"] < .15
                ):
                    original_frontier = self.frontiers.get(str(resolution["frontier_id"]))
                    original_branch = next(
                        (
                            candidate for candidate in original_frontier.branches.values()
                            if candidate.branch_id == resolution["branch_id"]
                        ),
                        None,
                    ) if original_frontier else None
                    original_bad = self._branch_values(original_branch, step)[4] if original_branch else 0.0
                    if original_frontier:
                        trigger = self._make_trigger(
                            "CLOSED_ROUTE_WITHOUT_ADVANCE",
                            step,
                            original_frontier,
                            resolution["source_descriptor"],
                            max(.75, original_bad),
                            1.0,
                            float(resolution["route_anchor_gain"]),
                            {
                                "branch": resolution["branch_label"],
                                "branch_id": resolution["branch_id"],
                                "route_length": resolution["route_length"],
                            },
                        )
                        if self._enqueue(trigger): enqueued.append(trigger.trigger_id)
        visits = len([x for x in source.recent_visit_steps if step - x <= 7])
        resolved = sum(
            1
            for receipt_item in self.attempt_receipts
            if receipt_item.frontier_id == source.frontier_id
            and receipt_item.resolved_outcome != "DEPARTURE_PENDING"
        )
        trusted_escape = any(b.escape_confidence >= .55 and b.raw_durable_count for b in source.branches.values())
        active_higher_priority = any(
            trigger.kind in {"BAD_BRANCH_REPEAT", "CLOSED_ROUTE_WITHOUT_ADVANCE"}
        ...
        if (
            ...
            and visits >= 3
            and resolved >= 2
            and not trusted_escape
            and gain < .15
            and not enqueued
            and not active_higher_priority
        ):
            trigger = self._make_trigger("FRONTIER_COLLAPSE", step, source, before_desc, min(1.0, (visits - 1) / 3), 0.0, gain, {"visits": visits})
            if self._enqueue(trigger): enqueued.append(trigger.trigger_id)
        if action.get("type") == "type_text":
            ...
            prior = next((x for x in reversed(occurrences) if step - x["step"] <= 12 and x["open_mask"] == old_mask), None)
            ...
            bad_prior = bool(
                prior
                and (
                    prior["outcome"] in {"NO_PROGRESS_EXACT", "NO_PROGRESS_NEGLIGIBLE", "RETURNED", "LATE_RETURN"}
                    or reentered_without_gain
                )
                and (not clear_between or prior["frontier_id"] == source.frontier_id)
            )
            if bad_prior and not phase_switch:
                trigger = self._make_trigger("VALUE_REENTRY_AFTER_BAD_OUTCOME", step, source, before_desc, max(.65, bad), r, gain, {"branch_id": branch.branch_id})
                if self._enqueue(trigger): enqueued.append(trigger.trigger_id)
            occurrences.append({
                "step": step,
                "open_mask": old_mask,
        ...
        if intent == "COMMIT" and len(self.anchors) >= 2 and commit_gains:
            gained_index, gained_anchor, _ = max(commit_gains, key=lambda item: (item[2], -item[0]))
            remaining_open_mask = new_mask & ~(1 << gained_index)
        ...
        for watch in self.escape_watches:
            ...
            max_open_gain = max((a.confidence - watch.base_confidences[i] for i, a in enumerate(self.anchors) if watch.open_anchor_mask & (1 << i)), default=0.0)
            if max_open_gain >= .10:
                continue
            if watch.offscreen_count >= 2 and max_open_gain < .10:
                ...
                if wf:
                    ...
                    trigger = self._make_trigger("PARTIAL_OBLIGATION_ESCAPE", step, destination, after_desc, anchor.confidence if anchor else .65, 0.0, max_open_gain, {"anchor": anchor.literal if anchor else "an item"})
                    if self._enqueue(trigger): enqueued.append(trigger.trigger_id)
            elif age < 4:
                mature.append(watch)
    ...
    def audit_record(self) -> dict[str, Any]:
        ...
        record: dict[str, Any] = {
            "schema": "a10_ecobf_audit_v1", "mechanism_id": self.mechanism_id,
            "experiment_id": self.experiment_id, "version": 1,
            "parameters": {"max_anchors": self.max_anchors, "max_anchor_events": self.max_anchor_events, "max_frontiers": self.max_frontiers, "max_branches_per_frontier": self.max_branches_per_frontier, "max_attempt_receipts": self.max_attempt_receipts, "max_pending_routes": self.max_pending_routes, "max_escape_watches": self.max_escape_watches, "max_trigger_candidates": self.max_trigger_candidates, "max_nonempty_reads": self.max_nonempty_reads, "max_reads_per_phase": self.max_reads_per_phase, "read_cooldown_steps": self.read_cooldown_steps, "max_chars": self.max_chars, "max_utf8_bytes": self.max_utf8_bytes, "visual_thresholds": {"changed_fraction": .001, "dl": .06, "de": .12, "dv_match": .055, "dv_merge": .035}, "confidence_thresholds": {"open": .35, "touched": .60, "provisional": .80, "retrieval": .68}, "route_horizons": {"return": 4, "late_return": 8}},
            ...
            "goal": {"goal_sha256": self.goal_sha256, "operation_class": self.operation_class, "anchor_count": len(anchors), "anchors": anchors},
            ...
            "capacity": {"max_observed_frontiers": self.max_observed_frontiers, "max_observed_branches": self.max_observed_branches, "max_observed_receipts": self.max_observed_receipts, "max_observed_rendered_chars": self.max_observed_rendered_chars, "max_observed_rendered_utf8_bytes": self.max_observed_rendered_utf8_bytes, "serialized_audit_bytes": 0},
