"""Fail-closed contract for the prospective A1-R4 WRPL arm."""
...
from .a1r4_writer_resilient_pending import EXPERIMENT_ID, MECHANISM_ID
...
PORT = base.PORT
PARENT_EVIDENCE_COMMIT = "4a28f757d7312f9ee87c84f4d750a3c43740de28"
CONFIG_SCHEMA = "a1r4_writer_resilient_pending_config_v1"
OFFLINE_REPLAY_SCHEMA = "a1r4_writer_resilient_pending_offline_replay_v1"
PREFLIGHT_SCHEMA = "a1r4_wrpl_zero_generation_preflight_v1"
LIVE_RECEIPT_SCHEMA = "a1r4_wrpl_live_server_receipt_v1"
RESULT_SCHEMA = "a1r4_wrpl_result_v1"
CHECKPOINT_SCHEMA = "a1r4_wrpl_checkpoint_v1"

REPOSITORY_ROOT = base.REPOSITORY_ROOT
CONFIG_PATH = REPOSITORY_ROOT / "implementation/configs/a1r4_writer_resilient_pending_hard_seed20260806.json"
OFFLINE_REPLAY_PATH = REPOSITORY_ROOT / "evidence/a1r4/A1R4_WRPL_OFFLINE_REPLAY_REPORT.json"
SOURCE_FREEZE_PATH = REPOSITORY_ROOT / "evidence/a1r4/A1R4_WRPL_SOURCE_FREEZE.json"
PREFLIGHT_PATH = REPOSITORY_ROOT / "evidence/a1r4/A1R4_WRPL_ZERO_GENERATION_PREFLIGHT.json"
...
SOURCE_FILES = (
    "protocols/A1R4_WRITER_RESILIENT_PENDING_PREREG_2026-08-15.md",
    "implementation/configs/a1r4_writer_resilient_pending_hard_seed20260806.json",
    ...
    "implementation/src/raven_m/official_qwen_mobile/a1r4_contract.py",
    ...
    "implementation/scripts/run_a1r4_wrpl.py",
    "implementation/scripts/replay_a1r4_writer_resilient_pending.py",
    ...
    "implementation/tests/official_qwen_mobile/test_a1r4_writer_resilient_pending.py",
    "implementation/tests/official_qwen_mobile/test_a1r4_contract.py",
    "implementation/tests/official_qwen_mobile/test_a1r4_controller_integration.py",
    "implementation/tests/official_qwen_mobile/test_a1r4_offline_replay.py",
    "evidence/a1r4/A1R4_WRPL_OFFLINE_REPLAY_REPORT.json",
    "evidence/a1r3/A1R3_SRPL_PRIMARY_GATE_RESULT_2026-08-15.json",
...
def source_freeze_payload(implementation_commit: str) -> dict[str, Any]:
    if len(implementation_commit) != 40 or any(
        ...
        raise RuntimeError("A1-R4 implementation commit invalid")
    if subprocess.run(
        ["git", "-C", str(REPOSITORY_ROOT), "merge-base", "--is-ancestor", PARENT_EVIDENCE_COMMIT, implementation_commit],
        capture_output=True,
    ).returncode:
        raise RuntimeError("A1-R4 parent evidence is not an ancestor")
    files: dict[str, str] = {}
    for name in SOURCE_FILES:
        path = REPOSITORY_ROOT / name
        if not path.is_file():
            raise RuntimeError(f"A1-R4 source closure missing: {name}")
        try:
            frozen = subprocess.check_output(
                ["git", "-C", str(REPOSITORY_ROOT), "show", f"{implementation_commit}:{name}"]
            )
        except subprocess.CalledProcessError as exc:
            raise RuntimeError(f"A1-R4 source absent from implementation commit: {name}") from exc
        if _git("hash-object", "--path", name, str(path)) != _git(
            "rev-parse", f"{implementation_commit}:{name}"
        ):
            raise RuntimeError(f"A1-R4 current source drift: {name}")
        files[name] = sha256(frozen).hexdigest()
    payload = {
        "schema": "a1r4_wrpl_source_freeze_v1",
...
def validate_source_freeze(path: Path = SOURCE_FREEZE_PATH) -> dict[str, Any]:
    ...
    if report != expected:
        raise RuntimeError("A1-R4 source freeze mismatch")
    return report
...
def validate_preflight_report(path: Path = PREFLIGHT_PATH) -> dict[str, Any]:
    ...
    if errors:
        raise RuntimeError(f"A1-R4 preflight invalid: {errors}")
    return report
...
def validate_launch_receipt(path: Path, *, preflight_path: Path = PREFLIGHT_PATH) -> dict[str, Any]:
    ...
    if errors:
        raise RuntimeError(f"A1-R4 receipt invalid: {errors}")
    return receipt
...
def exact_completion_errors(
    *, summaries: list[dict[str, Any]], invalid_attempts: list[dict[str, Any]], lifecycle_errors: list[dict[str, Any]]
) -> list[str]:
    return base.exact_completion_errors(
        summaries=summaries,
