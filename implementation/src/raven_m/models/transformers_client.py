...
class TransformersClient:
    ...
    def health(self) -> dict[str, Any]:
        ...
        if health.get("model") != self.model_id:
            raise RuntimeError(f"Unexpected model service: {health!r}")
        if health.get("revision") != self.model_revision:
            raise RuntimeError(f"Unexpected model revision: {health!r}")
        if health.get("backend") != self.backend_id:
    ...
    def generate(
        ...
        *,
        ...
        user_prompt_before_image: bool = False,
        current_image_label: str | None = "CURRENT_SCREENSHOT:",
    ) -> ModelCall:
        ...
        if user_prompt_before_image:
            user_content.append({"type": "text", "text": user_prompt})
            if current_image_label:
                user_content.append({"type": "text", "text": current_image_label})
            user_content.append(current_image)
        else:
            if current_image_label:
                user_content.append({"type": "text", "text": current_image_label})
            user_content.extend([current_image, {"type": "text", "text": user_prompt}])
        messages = [
        ...
        request_audit = {
            ...
            "context_image_labels": [label for label, _ in context_images],
            "max_tokens": max_tokens,
            "temperature": 0,
        }
        # Preserve the protocol-v1 request hash for all existing callers.
        if user_prompt_before_image or current_image_label != "CURRENT_SCREENSHOT:":
            request_audit.update(
                user_prompt_before_image=user_prompt_before_image,
                current_image_label=current_image_label,
            )
        request_sha = sha256(
            json.dumps(
                request_audit,
        ...
        if meta.get("model_revision") != self.model_revision:
            raise RuntimeError("Model service revision drift detected.")
        if meta.get("backend_id") != self.backend_id:
