#!/usr/bin/env python3
"""Qualify an A1-R3 vLLM process without a generation request."""
...
def main() -> int:
    ...
    if not manifest.is_file() or contract.file_sha256(manifest) != contract.MODEL_MANIFEST_SHA256:
        raise RuntimeError("A1-R3 live model manifest drift")
    ...
    if "vllm" not in cmdline or contract.MODEL_REALPATH not in cmdline or str(args.port) not in cmdline:
        raise RuntimeError("A1-R3 live process cmdline drift")
