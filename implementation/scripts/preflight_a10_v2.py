#!/usr/bin/env python3
"""Strict zero-generation preflight for the prospective A10-v2 arm."""
...
from raven_m.official_qwen_mobile.a10_v2_contract import (  # noqa: E402
    CONFIG_PATH, CONFIG_SCHEMA, DESIGN_PARENT_COMMIT, EXPERIMENT_ID, MECHANISM_ID,
    MODEL_ID, MODEL_REVISION, OFFICIAL_SYSTEM_PROMPT_SHA256, TASK_COUNT, TASK_SEED,
    current_source_freeze, json_sha256,
)
from raven_m.official_qwen_mobile.protocol import OFFICIAL_SYSTEM_PROMPT  # noqa: E402
from replay_a10_v2_offline_traces import verify_and_replay  # noqa: E402
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "evidence/a10_v2/A10_V2_ZERO_GENERATION_PREFLIGHT.json")
    ...
    if subprocess.run(["git", "-C", str(ROOT), "merge-base", "--is-ancestor", DESIGN_PARENT_COMMIT, args.implementation_commit], check=False).returncode:
        errors.append("design_parent_not_ancestor")
    ...
    if config.get("schema") != CONFIG_SCHEMA or config.get("experiment_id") != EXPERIMENT_ID or config.get("mechanism_id") != MECHANISM_ID:
    ...
    core = ROOT / "implementation/src/raven_m/official_qwen_mobile/a10_v2_obligation_branch_frontier.py"
    ...
    if not args.tokenizer_path.is_dir():
        ...
        try:
            ...
            samples = ['Open: "zucchini in directions". Evidence: repeated visible no-progress; reassess another action family.']
            maximum = max(len(tokenizer.encode(item, add_special_tokens=False)) for item in samples)
    ...
    report = {"schema": "a10_v2_zero_generation_preflight_v1", "status": "pass" if not errors else "fail", "generation_calls": 0, "created_at": datetime.now(timezone.utc).isoformat(), "design_parent_commit": DESIGN_PARENT_COMMIT, "implementation_commit": args.implementation_commit, "mechanism_id": MECHANISM_ID, "experiment_id": EXPERIMENT_ID, "source_freeze": freeze, "source_freeze_sha256": json_sha256(freeze), "checks": checks, "errors": errors}
