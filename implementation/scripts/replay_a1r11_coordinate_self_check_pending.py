...
def replay(s:Path)->dict:
 ...
 try:base.GoalAnchoredPendingMemory=CoordinateSelfCheckPendingMemory;result=base.replay(s)
 finally:base.GoalAnchoredPendingMemory=old
 result["schema"]="a1r11_coordinate_self_check_pending_offline_replay_v1";result["mechanism_id"]=MECHANISM_ID;result["content_sha256"]=base.base.base._content_sha(result);return result
def main()->int:
 p=argparse.ArgumentParser();p.add_argument("--suite-dir",type=Path,default=base.base.base.DEFAULT_SUITE);p.add_argument("--output",type=Path,default=ROOT/"evidence/a1r11/A1R11_CSCP_OFFLINE_REPLAY_REPORT.json");a=p.parse_args();r=replay(a.suite_dir.resolve());a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(r,sort_keys=True,indent=2)+"\n");print(r["status"],r["errors"]);return 0 if r["status"]=="PASS" else 1
