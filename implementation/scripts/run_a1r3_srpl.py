#!/usr/bin/env python3
"""Build or execute the frozen A1-R3 SRPL command."""
...
def main() -> int:
    ...
    command = [
        ...
        "--a1r3-srpl",
        "--a1r3-preflight-report", str(ROOT / "evidence/a1r3/A1R3_SRPL_ZERO_GENERATION_PREFLIGHT.json"),
        "--a1r3-launch-receipt", str(args.launch_receipt.resolve()),
        "--output-root", str(ROOT / "runs/a1r3_srpl"),
    ]
    if args.resume_suite_dir is not None:
        command.extend(["--resume-suite-dir", str(args.resume_suite_dir.resolve())])
    print(subprocess.list2cmdline(command))
    if not args.execute:
        print("Dry run only. Use --execute only after the A1-R3 receipt passes.")
        return 0
