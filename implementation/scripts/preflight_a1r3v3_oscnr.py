#!/usr/bin/env python3
"""Strict zero-generation preflight for A1-R3-v3 OSCNR."""
...
from replay_a1r3v3_one_shot_cnr import DEFAULT_TOKENIZER, replay  # noqa: E402
...
def _stable_replay(value: dict) -> dict:
    source = dict(value.get("source") or {})
    manifest = dict(source.get("trace_manifest") or {})
    manifest.pop("files", None)
    source["trace_manifest"] = manifest
    source.pop("trace_manifest_file_sha256", None)
    return {
        "schema": value.get("schema"),
        "status": value.get("status"),
        "errors": value.get("errors"),
        "generation_calls": value.get("generation_calls"),
        "development_calibration_not_confirmation": value.get("development_calibration_not_confirmation"),
...
def main() -> int:
    ...
    if status:
        errors.append("worktree_dirty_before_generated_evidence")
    ...
    replay_now = replay(
        ROOT / "runs/a1r2_cvp/official_qwen_20260814T145307_50081981",
        tokenizer_path=args.tokenizer_path.resolve(),
        include_manifest_files=False,
    )
    ...
    if replay_now["status"] != "PASS" or _stable_replay(replay_now) != _stable_replay(replay_file):
        errors.append("offline_replay_drift")
    core = ROOT / "implementation/src/raven_m/official_qwen_mobile/a1r3v3_one_shot_cnr.py"
    ...
    for _ in range(1000):
        ...
        for step in range(2):
            memory.observe_step(
                ...
                source_call_id=f"x{step}", source_response_sha256=f"x{step}",
                source_screenshot_sha256=f"b{step}", source_after_screenshot_sha256=f"a{step}",
            )
