...
from implementation.scripts.replay_a1r14_response_value_register import replay  # noqa: E402
...
def main() -> int:
    ...
    tests = subprocess.run([sys.executable, "-m", "pytest", "-q", "-p", "no:cacheprovider", "implementation/tests/official_qwen_mobile/test_a1r14_response_value_register.py", "implementation/tests/official_qwen_mobile/test_a1r14_contract.py", "implementation/tests/official_qwen_mobile/test_a1r14_controller_integration.py"], cwd=ROOT, env=env, capture_output=True, text=True)
    ...
    payload = {"schema": contract.PREFLIGHT_SCHEMA, "status": "PASS" if not errors else "FAIL", "errors": errors, "generation_calls": 0, "live_generation_authorized": not errors, "mechanism_id": contract.MECHANISM_ID, "experiment_id": contract.EXPERIMENT_ID, "implementation_commit": args.implementation_commit, "source_freeze_content_sha256": freeze.get("content_sha256"), "offline_replay_content_sha256": replay_file.get("content_sha256"), "fixture_content_sha256": fixture.get("content_sha256"), "checks": checks, "created_at": datetime.now(timezone.utc).isoformat()}
