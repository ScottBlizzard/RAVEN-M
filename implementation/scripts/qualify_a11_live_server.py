#!/usr/bin/env python3
"""Issue an A11 receipt only for a matching live process and passed preflight."""
...
from raven_m.official_qwen_mobile.a11_contract import validate_preflight_report  # noqa: E402
...
def main() -> None:
    ...
    if intent.get("schema") != "a11_server_launch_intent_v1" or intent.get("arm") != "a11": raise RuntimeError("A11 intent identity mismatch")
    if intent.get("preflight_sha256") != digest(args.preflight) or intent.get("source_freeze_sha256") != preflight["source_freeze_sha256"]: raise RuntimeError("A11 intent provenance mismatch")
    ...
    result = {"schema": "a11_live_server_receipt_v1", "status": "pass", "generation_calls": 0, "a11_preflight_sha256": digest(args.preflight), "a11_source_freeze_sha256": preflight["source_freeze_sha256"], "launch_intent_sha256": digest(args.launch_intent), "launch_intent_path": str(args.launch_intent.resolve()), "served_model_id": intent["served_model_id"], "model_realpath": intent["model_realpath"], "model_manifest_sha256": intent["model_manifest_sha256"], "pid": pid, "process_pid": pid, "process_cmdline": cmdline, "port": int(intent["port"]), "packages": packages, "served_model_ids_observed": ids, "qualification_timestamp": datetime.now(timezone.utc).isoformat()}
