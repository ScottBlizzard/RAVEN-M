#!/usr/bin/env python3
"""Materialize the A1-R15 replay fixture from sealed local evidence."""
...
def materialize(v4_suite: Path, target_suite: Path) -> dict:
    ...
    sources[order_contract.TARGET_GATE_TASK] = _episodes(target_suite)[order_contract.TARGET_GATE_TASK]
    rows = []
    ...
    payload = {
        "schema": "a1r15_eovr_replay_fixture_v1",
        "generation_calls": 0,
        "source_policy": "SYS-NAG-V4-19 with Browser replaced by sealed A1-R14 target trace",
...
def main() -> int:
    ...
    parser.add_argument("--target-suite", type=Path, default=_latest(ROOT / "runs/a1r14_rgvr"))
    parser.add_argument("--output", type=Path, default=ROOT / "evidence/a1r15/A1R15_EOVR_REPLAY_FIXTURE.json")
