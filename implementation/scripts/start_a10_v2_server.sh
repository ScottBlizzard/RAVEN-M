...
PREFLIGHT="${A10_V2_PREFLIGHT:-${REPO_DIR}/evidence/a10_v2/A10_V2_ZERO_GENERATION_PREFLIGHT.json}"
INTENT="${A10_V2_SERVER_LAUNCH_INTENT:-${REPO_DIR}/evidence/a10_v2/A10_V2_SERVER_LAUNCH_INTENT.json}"
PORT="${RAVEN_SERVER_PORT:-18000}"
export PYTHONPATH="${REPO_DIR}/implementation/src"
ARM=a10v2 SCHEMA=a10_v2_server_launch_intent_v1 PREFLIGHT="${PREFLIGHT}" INTENT="${INTENT}" MODEL_DIR="${MODEL_DIR}" ENV_DIR="${ENV_DIR}" PORT="${PORT}" "${ENV_DIR}/bin/python" - <<'PY'
import importlib.metadata, json, os
from hashlib import sha256
from pathlib import Path
from raven_m.official_qwen_mobile.a10_v2_contract import MODEL_ID, MODEL_MANIFEST_SHA256, validate_preflight_report
