#!/usr/bin/env python3
"""Zero-generation replay of A1-R2 traces through A1-R4 WRPL."""
...
from implementation.scripts.replay_a1r3_stale_resistant_pending import (  # noqa: E402
    A1R2_SUITE_ID,
    DEFAULT_SUITE,
    SENTINELS,
    _canonical_sha,
...
def replay(suite_dir: Path) -> dict[str, Any]:
    ...
    for summary in summaries:
        ...
        for step in summary.get("steps") or []:
            ...
            if rendered:
                reminder_reads += int(read.get("writer_reminder_injected") is True)
                semantic_reads += int(read.get("semantic_memory_injected") is True)
            ...
            memory.observe_step(
                ...
                source_screenshot_sha256=str(
                    step.get("before_screenshot_sha256") or "offline-replay"
                ),
            )
        ...
        episodes.append(
            {
                ...
                "writer_reminder_reads": reminder_reads,
                "semantic_memory_reads": semantic_reads,
    ...
    totals = {
        ...
        "writer_reminder_reads": sum(item["writer_reminder_reads"] for item in episodes),
        "semantic_memory_reads": sum(item["semantic_memory_reads"] for item in episodes),
        ...
        "failure_evidence_read_count": sum(
            item["failure_evidence_read_count"] for item in episodes
        ),
    ...
    if totals["writer_reminder_reads"] < 19 or totals["semantic_memory_reads"] < 1:
        errors.append("writer_semantic_exposure")
    ...
    report = {
        "schema": "a1r4_writer_resilient_pending_offline_replay_v1",
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=ROOT / "evidence/a1r4/A1R4_WRPL_OFFLINE_REPLAY_REPORT.json",
    )
