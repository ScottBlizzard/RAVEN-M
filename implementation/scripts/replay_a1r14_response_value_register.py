#!/usr/bin/env python3
"""Zero-generation replay for A1-R14 response-grounded value memory."""
...
from raven_m.official_qwen_mobile.a1r14_response_value_register import (  # noqa: E402
    EXPERIMENT_ID,
    MECHANISM_ID,
...
def replay(fixture_path: Path) -> dict:
    ...
    errors: list[str] = []
    if fixture.get("schema") != "a1r14_rgvr_replay_fixture_v1" or fixture.get("content_sha256") != digest_contract.content_sha256(fixture):
    ...
    if len(rows) != 19 or [row["task_name"] for row in rows] != list(order_contract.FULL_TASK_ORDER):
        errors.append("task_order")
    if [row["task_name"] for row in active] != [order_contract.TARGET_GATE_TASK]:
        errors.append("activation_specificity")
    if success_active:
        errors.append("success_silence")
    if target.get("final_values") != ["1", "8", "10", "7", "2"]:
        errors.append("target_values")
    if int(target.get("response_append_count") or 0) != 5:
        errors.append("target_response_append_count")
    if not any("observed integer sequence = [1, 8, 10, 7, 2]." in row["text"] for row in target.get("rendered_reads") or []):
        errors.append("target_exact_read")
    totals = {
        "episode_count": len(rows),
        "step_count": sum(len(row.get("steps") or []) for row in fixture.get("episodes") or []),
        "active_episode_count": len(active),
        "six_success_active_count": len(success_active),
    ...
    payload = {
        "schema": "a1r14_rgvr_offline_replay_v1",
...
def main() -> int:
    ...
    parser.add_argument("--fixture", type=Path, default=ROOT / "evidence/a1r14/A1R14_RGVR_REPLAY_FIXTURE.json")
    parser.add_argument("--output", type=Path, default=ROOT / "evidence/a1r14/A1R14_RGVR_OFFLINE_REPLAY_REPORT.json")
