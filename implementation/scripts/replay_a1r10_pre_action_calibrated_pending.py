...
def replay(s:Path)->dict:
 ...
 try:base.GoalAnchoredPendingMemory=PreActionCalibratedPendingMemory;r=base.replay(s)
 finally:base.GoalAnchoredPendingMemory=o
 r["schema"]="a1r10_pre_action_calibrated_pending_offline_replay_v1";r["mechanism_id"]=MECHANISM_ID;r["content_sha256"]=base.base.base._content_sha(r);return r
def main()->int:
 p=argparse.ArgumentParser();p.add_argument("--suite-dir",type=Path,default=base.base.base.DEFAULT_SUITE);p.add_argument("--output",type=Path,default=ROOT/"evidence/a1r10/A1R10_PACP_OFFLINE_REPLAY_REPORT.json");a=p.parse_args();r=replay(a.suite_dir.resolve());a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(r,sort_keys=True,indent=2)+"\n");print(r["status"],r["errors"]);return 0 if r["status"]=="PASS" else 1
