#!/usr/bin/env python3
"""Build or execute the frozen A1-R2 CVP command."""
...
def main() -> int:
    ...
    command = [
        ...
        "--a1r2-cvp",
        "--a1r2-preflight-report", str(ROOT / "evidence/a1r2/A1R2_CVP_ZERO_GENERATION_PREFLIGHT.json"),
        "--a1r2-launch-receipt", str(args.launch_receipt.resolve()),
        "--output-root", str(ROOT / "runs/a1r2_cvp"),
    ]
    if args.resume_suite_dir is not None:
        command.extend(["--resume-suite-dir", str(args.resume_suite_dir.resolve())])
    print(subprocess.list2cmdline(command))
    if not args.execute:
        print("Dry run only. Use --execute only after the A1-R2 receipt passes.")
        return 0
