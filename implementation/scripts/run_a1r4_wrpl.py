#!/usr/bin/env python3
"""Build or execute the frozen A1-R4 WRPL command."""
...
def main() -> int:
    ...
    command = [
        ...
        "--a1r4-wrpl",
        "--a1r4-preflight-report", str(ROOT / "evidence/a1r4/A1R4_WRPL_ZERO_GENERATION_PREFLIGHT.json"),
        "--a1r4-launch-receipt", str(args.launch_receipt.resolve()),
        "--output-root", str(ROOT / "runs/a1r4_wrpl"),
    ]
    if args.resume_suite_dir is not None:
        command.extend(["--resume-suite-dir", str(args.resume_suite_dir.resolve())])
    print(subprocess.list2cmdline(command))
    if not args.execute:
        print("Dry run only. Use --execute only after the A1-R4 receipt passes.")
        return 0
