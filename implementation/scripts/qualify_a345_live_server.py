"""Bind A3/A4/A5 scored runs to the live vLLM process and frozen preflight."""
...
def main() -> None:
    ...
    if preflight.get("status") != "pass" or preflight.get("generation_calls") != 0:
        raise RuntimeError("A345 zero-generation preflight has not passed")
    ...
    result = {
        "schema": "a345_live_server_receipt_v1",
        "status": "pass",
        "generation_calls": 0,
        "pid": pid,
        "process_cmdline": cmdline,
        "launch_intent_sha256": _hash(args.launch_intent),
        "a345_preflight_sha256": _hash(args.preflight),
    ...
    print(json.dumps({"status": "pass", "output": str(args.output), "sha256": _hash(args.output)}, indent=2))
