...
| AndroidWorld Hard task 评测 | 官方 task-list 快照、19-task manifest、原生预算、三组 seed | 协议审计 19/19、G7 与 protocol-v1 冻结检查均通过 | 协议已冻结；正式计分尚未启动 |
| 多智能体记忆框架 | Planner/Executor/Memory/Critic、类型化 memory、证据路由与失效 | 代码、schema、append-only events、角色调用审计 | 已实现；失败开发套件完整保留，v15 通过 G7 |
| 历史、状态摘要、经验复用、多轮规划 | B0 raw、B1/B2 sliding、B3 summary、S0/M0 结构化 memory | G4 公平基线、S0/M0、8 路组件 smoke 与检索审计均已完成 | 非 Hard 开发与冻结证据已完成 |
