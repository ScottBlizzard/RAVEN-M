...
OVERLAY = (
    ROOT / "05_project/configs/experiments/v2_2_h17_candidate_r63.json"
)
WRAPPER = (
    ROOT / "05_project/scripts/"
    "run_protocol_v2_2_r63_h17_candidate_smoke.py"
)
RUNNER = ROOT / "05_project/scripts/run_protocol_v2_gate_f.py"
R62_STOP_REPORT = (
    ROOT / "reports/protocol_v2_2_r62_h17_candidate_stopped.json"
)
R63_LOCAL_REPORT = (
    ROOT / "reports/protocol_v2_2_r63_local_validation.json"
)
...
def test_r63_candidate_preserves_frozen_experiment_controls() -> None:
    wrapper = load_wrapper("r63_candidate_controls")
...
def test_r63_candidate_freeze_hashes_match_exact_source_commit() -> None:
    wrapper = load_wrapper("r63_candidate_freeze")
    manifest = json.loads(
        wrapper.build_candidate_manifest().read_text(encoding="utf-8")
    )
...
def test_r63_historical_static_validation_rejects_future_head_drift() -> None:
    wrapper = load_wrapper("r63_candidate_static_wrapper")
    runner = load_module(RUNNER, "r63_candidate_static_runner")
    manifest = json.loads(
        wrapper.build_candidate_manifest().read_text(encoding="utf-8")
    )
    with pytest.raises(
...
def test_r63_candidate_prerequisites_are_byte_exact_and_fail_closed() -> None:
    overlay = json.loads(OVERLAY.read_text(encoding="utf-8"))
    assert sha256(R62_STOP_REPORT.read_bytes()).hexdigest() == (
        overlay["prerequisite_r62_stop_report"]["sha256"]
    )
    stopped = json.loads(R62_STOP_REPORT.read_text(encoding="utf-8"))
    ...
    assert sha256(R63_LOCAL_REPORT.read_bytes()).hexdigest() == (
        overlay["prerequisite_r63_local_validation"]["sha256"]
    )
    local = json.loads(R63_LOCAL_REPORT.read_text(encoding="utf-8"))
...
def test_r63_candidate_wrapper_rejects_every_other_live_scope(
    argv: list[str],
) -> None:
    wrapper = load_wrapper("r63_candidate_invalid_invocation")
    with pytest.raises(RuntimeError):
        wrapper.validate_invocation(argv)


def test_r63_candidate_wrapper_allows_only_preflight_or_h17_m0() -> None:
    wrapper = load_wrapper("r63_candidate_valid_invocation")
...
def test_r63_candidate_wrapper_uses_non_formal_mode() -> None:
...
def test_r63_mechanism_sources_are_task_and_answer_agnostic() -> None:
    paths = [
