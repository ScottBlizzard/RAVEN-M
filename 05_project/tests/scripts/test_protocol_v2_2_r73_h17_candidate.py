...
OVERLAY = (
    ROOT / "05_project/configs/experiments/v2_2_h17_candidate_r73.json"
)
WRAPPER = (
    ROOT / "05_project/scripts/"
    "run_protocol_v2_2_r73_h17_candidate_smoke.py"
)
RUNNER = ROOT / "05_project/scripts/run_protocol_v2_gate_f.py"
R72_STOP_REPORT = (
    ROOT / "reports/protocol_v2_2_r72_h17_candidate_stopped.json"
)
R73_LOCAL_REPORT = (
    ROOT / "reports/protocol_v2_2_r73_local_validation.json"
)
...
def test_r73_candidate_preserves_frozen_experiment_controls() -> None:
    wrapper = load_wrapper("r73_candidate_controls")
...
def test_r73_candidate_freeze_hashes_match_exact_source_commit() -> None:
    wrapper = load_wrapper("r73_candidate_freeze")
    manifest = json.loads(
        wrapper.build_candidate_manifest().read_text(encoding="utf-8")
    )
...
def test_r73_historical_static_validation_rejects_future_head_drift() -> None:
    wrapper = load_wrapper("r73_candidate_static_wrapper")
    runner = load_module(RUNNER, "r73_candidate_static_runner")
    manifest = json.loads(
        wrapper.build_candidate_manifest().read_text(encoding="utf-8")
    )
    with pytest.raises(
...
def test_r73_candidate_prerequisites_are_byte_exact() -> None:
    overlay = json.loads(OVERLAY.read_text(encoding="utf-8"))
    assert sha256(R72_STOP_REPORT.read_bytes()).hexdigest() == (
        overlay["prerequisite_r72_stop_report"]["sha256"]
    )
    stopped = json.loads(R72_STOP_REPORT.read_text(encoding="utf-8"))
    ...
    assert sha256(R73_LOCAL_REPORT.read_bytes()).hexdigest() == (
        overlay["prerequisite_r73_local_validation"]["sha256"]
    )
    local = json.loads(R73_LOCAL_REPORT.read_text(encoding="utf-8"))
...
def test_r73_candidate_wrapper_rejects_every_other_live_scope(
    argv: list[str],
) -> None:
    wrapper = load_wrapper("r73_candidate_invalid_invocation")
    with pytest.raises(RuntimeError):
        wrapper.validate_invocation(argv)


def test_r73_candidate_wrapper_allows_only_preflight_or_h17_m0() -> None:
    wrapper = load_wrapper("r73_candidate_valid_invocation")
...
def test_r73_candidate_wrapper_uses_non_formal_mode() -> None:
...
def test_r73_mechanism_is_narrow_and_leaves_policy_unchanged() -> None:
    controller = (
        ROOT / "05_project/src/raven_m/controller/episode_controller.py"
    ).read_text(encoding="utf-8")
    overlay = json.loads(OVERLAY.read_text(encoding="utf-8"))
    assert "_verified_active_detail_inspection_action" in controller
    assert 'action.get("type") == "tap"' in controller
    assert 'assessment.get("inspection_control_hit") is True' in controller
    assert 'assessment.get("mutation_control_hit") is False' in controller
    assert (
        'assessment.get("requested_field_control_hit") is False'
        in controller
    )
    assert 'assessment.get("type_text_attempted") is False' in controller
    policy = ROOT / "05_project/src/raven_m/history/policies.py"
    assert sha256(policy.read_bytes()).hexdigest() == (
        overlay["updated_freeze_hashes"][
            "05_project/src/raven_m/history/policies.py"
        ]
    )


def test_r73_mechanism_sources_are_task_and_answer_agnostic() -> None:
    paths = [
        ROOT / "05_project/src/raven_m/controller/episode_controller.py",
