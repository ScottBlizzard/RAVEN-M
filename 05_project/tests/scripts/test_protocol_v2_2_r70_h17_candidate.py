...
OVERLAY = (
    ROOT / "05_project/configs/experiments/v2_2_h17_candidate_r70.json"
)
WRAPPER = (
    ROOT / "05_project/scripts/"
    "run_protocol_v2_2_r70_h17_candidate_smoke.py"
)
RUNNER = ROOT / "05_project/scripts/run_protocol_v2_gate_f.py"
R69_STOP_REPORT = (
    ROOT / "reports/protocol_v2_2_r69_h17_candidate_stopped.json"
)
R70_LOCAL_REPORT = (
    ROOT / "reports/protocol_v2_2_r70_local_validation.json"
)
...
def test_r70_candidate_preserves_frozen_experiment_controls() -> None:
    wrapper = load_wrapper("r70_candidate_controls")
...
def test_r70_candidate_freeze_hashes_match_exact_source_commit() -> None:
    wrapper = load_wrapper("r70_candidate_freeze")
    manifest = json.loads(
        wrapper.build_candidate_manifest().read_text(encoding="utf-8")
    )
...
def test_r70_historical_static_validation_rejects_future_head_drift() -> None:
    wrapper = load_wrapper("r70_candidate_static_wrapper")
    runner = load_module(RUNNER, "r70_candidate_static_runner")
    manifest = json.loads(
        wrapper.build_candidate_manifest().read_text(encoding="utf-8")
    )
    with pytest.raises(
...
def test_r70_candidate_prerequisites_are_byte_exact() -> None:
    overlay = json.loads(OVERLAY.read_text(encoding="utf-8"))
    assert sha256(R69_STOP_REPORT.read_bytes()).hexdigest() == (
        overlay["prerequisite_r69_stop_report"]["sha256"]
    )
    stopped = json.loads(R69_STOP_REPORT.read_text(encoding="utf-8"))
    ...
    assert sha256(R70_LOCAL_REPORT.read_bytes()).hexdigest() == (
        overlay["prerequisite_r70_local_validation"]["sha256"]
    )
    local = json.loads(R70_LOCAL_REPORT.read_text(encoding="utf-8"))
    assert local["source_commit"] == (
        "1ccd6290b4584478d0eda7e602584d149888968c"
    )
...
def test_r70_candidate_wrapper_rejects_every_other_live_scope(
    argv: list[str],
) -> None:
    wrapper = load_wrapper("r70_candidate_invalid_invocation")
    with pytest.raises(RuntimeError):
        wrapper.validate_invocation(argv)


def test_r70_candidate_wrapper_allows_only_preflight_or_h17_m0() -> None:
    wrapper = load_wrapper("r70_candidate_valid_invocation")
...
def test_r70_candidate_wrapper_uses_non_formal_mode() -> None:
...
def test_r70_mechanism_is_accessibility_first_and_pixel_bounded() -> None:
    guard = (
...
def test_r70_mechanism_sources_are_task_and_answer_agnostic() -> None:
    paths = [
        ROOT / "05_project/src/raven_m/controller/protocol_v2_guard.py",
        ROOT / "05_project/src/raven_m/controller/episode_controller.py",
