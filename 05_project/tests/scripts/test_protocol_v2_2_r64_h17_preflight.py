...
REPORT = (
    ROOT / "reports/protocol_v2_2_r64_h17_candidate_preflight.json"
)
REPORT_SHA256 = (
    "cb7fb064b32ad8de819b823ee653928e464a0bdaaa0bff0e78f8cf34e8127142"
)
...
def test_r64_h17_preflight_is_byte_frozen_and_zero_call() -> None:
    report = payload()
...
def test_r64_h17_preflight_matches_exact_source_and_package() -> None:
    report = payload()
    assert report["source_commit"] == (
        "223b54f72961bc59d4e84ea2ec26fe4124138cf3"
    )
    assert report["source_tag"] == "protocol-v2-2-r64-local-candidate"
    assert report["execution_commit"] == (
        "670639b5f230d526625ca993b33db86371ca12b6"
    )
    resolved = subprocess.check_output(
        [
            ...
            "-n",
            "1",
            "protocol-v2-2-r64-h17-candidate-package",
        ],
        cwd=ROOT,
        text=True,
    ).strip()
    assert resolved == "670639b5f230d526625ca993b33db86371ca12b6"


def test_r64_h17_preflight_freezes_all_files_and_pairs() -> None:
    report = payload()
...
def test_r64_h17_preflight_matches_exact_live_environment() -> None:
    report = payload()
...
def test_r64_h17_preflight_recorded_fresh_suite_absence() -> None:
