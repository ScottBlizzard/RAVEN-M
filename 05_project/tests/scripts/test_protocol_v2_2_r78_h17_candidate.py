...
OVERLAY = (
    ROOT / "05_project/configs/experiments/v2_2_h17_candidate_r78.json"
)
WRAPPER = (
    ROOT / "05_project/scripts/"
    "run_protocol_v2_2_r78_h17_candidate_smoke.py"
)
RUNNER = ROOT / "05_project/scripts/run_protocol_v2_gate_f.py"
R77_STOP_REPORT = (
    ROOT / "reports/protocol_v2_2_r77_h17_candidate_stopped.json"
)
R78_LOCAL_REPORT = (
    ROOT / "reports/protocol_v2_2_r78_local_validation.json"
)
...
def test_r78_candidate_preserves_frozen_experiment_controls() -> None:
    wrapper = load_wrapper("r78_candidate_controls")
...
def test_r78_candidate_freeze_hashes_match_exact_source_commit() -> None:
    wrapper = load_wrapper("r78_candidate_freeze")
    manifest = json.loads(
        wrapper.build_candidate_manifest().read_text(encoding="utf-8")
    )
...
def test_r78_candidate_static_manifest_validation_passes() -> None:
    wrapper = load_wrapper("r78_candidate_static_wrapper")
    runner = load_module(RUNNER, "r78_candidate_static_runner")
    manifest = json.loads(
        wrapper.build_candidate_manifest().read_text(encoding="utf-8")
    )
...
def test_r78_candidate_prerequisites_are_byte_exact() -> None:
    overlay = json.loads(OVERLAY.read_text(encoding="utf-8"))
    assert sha256(R77_STOP_REPORT.read_bytes()).hexdigest() == (
        overlay["prerequisite_r77_stop_report"]["sha256"]
    )
    stopped = json.loads(R77_STOP_REPORT.read_text(encoding="utf-8"))
    ...
    assert sha256(R78_LOCAL_REPORT.read_bytes()).hexdigest() == (
        overlay["prerequisite_r78_local_validation"]["sha256"]
    )
    local = json.loads(R78_LOCAL_REPORT.read_text(encoding="utf-8"))
    assert local["source_commit"] == (
        "69ab866b08f3af13f304bdc99f2af0a985cff0cf"
    )
...
def test_r78_candidate_wrapper_rejects_every_other_live_scope(
    argv: list[str],
) -> None:
    wrapper = load_wrapper("r78_candidate_invalid_invocation")
    with pytest.raises(RuntimeError):
        wrapper.validate_invocation(argv)


def test_r78_candidate_wrapper_allows_only_preflight_or_h17_m0() -> None:
    wrapper = load_wrapper("r78_candidate_valid_invocation")
...
def test_r78_candidate_wrapper_uses_non_formal_mode() -> None:
...
def test_r78_mechanism_is_terminal_bounded_and_fail_closed() -> None:
    controller = (
        ROOT / "05_project/src/raven_m/controller/episode_controller.py"
    ).read_text(encoding="utf-8")
    assert "terminal_routed_evidence_memory_projection.v1" in controller
    assert "TERMINAL_EVIDENCE_SCOPE" in controller
    assert "Terminal routed evidence requires complete target-row " in (
        controller
    )
    assert '"progress."' in controller
    assert "status=fail and action=null" in controller
    assert "COORDINATE_CHECK:" in controller
    assert "CHRONOLOGICAL_HISTORY_NAVIGATION:" in controller


def test_r78_mechanism_leaves_shared_frozen_sources_unchanged() -> None:
    overlay = json.loads(OVERLAY.read_text(encoding="utf-8"))
    for path in (
...
def test_r78_mechanism_sources_are_task_and_answer_agnostic() -> None:
    paths = [
        ROOT / "05_project/src/raven_m/controller/episode_controller.py",
