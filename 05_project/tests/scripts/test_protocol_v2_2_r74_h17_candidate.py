...
OVERLAY = (
    ROOT / "05_project/configs/experiments/v2_2_h17_candidate_r74.json"
)
WRAPPER = (
    ROOT / "05_project/scripts/"
    "run_protocol_v2_2_r74_h17_candidate_smoke.py"
)
RUNNER = ROOT / "05_project/scripts/run_protocol_v2_gate_f.py"
R73_STOP_REPORT = (
    ROOT / "reports/protocol_v2_2_r73_h17_candidate_stopped.json"
)
R74_LOCAL_REPORT = (
    ROOT / "reports/protocol_v2_2_r74_local_validation.json"
)
...
def test_r74_candidate_preserves_frozen_experiment_controls() -> None:
    wrapper = load_wrapper("r74_candidate_controls")
...
def test_r74_candidate_freeze_hashes_match_exact_source_commit() -> None:
    wrapper = load_wrapper("r74_candidate_freeze")
    manifest = json.loads(
        wrapper.build_candidate_manifest().read_text(encoding="utf-8")
    )
...
def test_r74_historical_static_validation_rejects_future_head_drift() -> None:
    wrapper = load_wrapper("r74_candidate_static_wrapper")
    runner = load_module(RUNNER, "r74_candidate_static_runner")
    manifest = json.loads(
        wrapper.build_candidate_manifest().read_text(encoding="utf-8")
    )
    with pytest.raises(
...
def test_r74_candidate_prerequisites_are_byte_exact() -> None:
    overlay = json.loads(OVERLAY.read_text(encoding="utf-8"))
    assert sha256(R73_STOP_REPORT.read_bytes()).hexdigest() == (
        overlay["prerequisite_r73_stop_report"]["sha256"]
    )
    stopped = json.loads(R73_STOP_REPORT.read_text(encoding="utf-8"))
    assert stopped["decision"] == (
        "r73_verified_inspection_and_native_popup_succeeded_but_"
        "nested_return_released_row_coordinates_early"
    )
    assert stopped["immutability"]["suite_may_be_resumed"] is False
    assert sha256(R74_LOCAL_REPORT.read_bytes()).hexdigest() == (
        overlay["prerequisite_r74_local_validation"]["sha256"]
    )
    local = json.loads(R74_LOCAL_REPORT.read_text(encoding="utf-8"))
    assert local["source_commit"] == (
        "b1b853920304057a52b1713e4a5a82dd98249dec"
    )
...
def test_r74_candidate_wrapper_rejects_every_other_live_scope(
    argv: list[str],
) -> None:
    wrapper = load_wrapper("r74_candidate_invalid_invocation")
    with pytest.raises(RuntimeError):
        wrapper.validate_invocation(argv)


def test_r74_candidate_wrapper_allows_only_preflight_or_h17_m0() -> None:
    wrapper = load_wrapper("r74_candidate_valid_invocation")
...
def test_r74_candidate_wrapper_uses_non_formal_mode() -> None:
...
def test_r74_mechanism_is_bounded_and_fails_closed() -> None:
    controller = (
    ...
    assert "return_to_target_date_list" in controller
    assert "reconcile_target_row_list_return" in controller
    assert "TARGET_ROW_LIST_RETURN_GUARD" in guard
    assert "TARGET_ROW_LIST_RETURN_FAILED" in guard
    assert "self.target_row_return_back_count >= 2" in guard
    assert 'action.get("type") != "press_back"' in guard
    assert "observed_row_count == self.target_date_row_count" in guard
    assert "== self.target_date_row_count" in guard


def test_r74_mechanism_leaves_unrelated_frozen_sources_unchanged() -> None:
    overlay = json.loads(OVERLAY.read_text(encoding="utf-8"))
    for path in (
...
def test_r74_mechanism_sources_are_task_and_answer_agnostic() -> None:
    paths = [
        ROOT / "05_project/src/raven_m/controller/episode_controller.py",
        ROOT / "05_project/src/raven_m/controller/protocol_v2_guard.py",
