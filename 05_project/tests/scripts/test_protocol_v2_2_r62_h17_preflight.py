...
REPORT = (
    ROOT / "reports/protocol_v2_2_r62_h17_candidate_preflight.json"
)
REPORT_SHA256 = (
    "ede56a77acb57f8607a84b78d62baee97996cfef4616c6baac16ed9313786b41"
)
...
def test_r62_h17_preflight_is_byte_frozen_and_zero_call() -> None:
    report = payload()
...
def test_r62_h17_preflight_matches_exact_source_and_package() -> None:
    report = payload()
    assert report["source_commit"] == (
        "4c194d0fd8edbf93447f27f7b663dc802622e563"
    )
    assert report["source_tag"] == "protocol-v2-2-r62-local-candidate"
    assert report["execution_commit"] == (
        "8b55ef1799478a721affc97ad38ed0aa48f2fe94"
    )
    resolved = subprocess.check_output(
        [
            ...
            "-n",
            "1",
            "protocol-v2-2-r62-h17-candidate-package",
        ],
        cwd=ROOT,
        text=True,
    ).strip()
    assert resolved == "8b55ef1799478a721affc97ad38ed0aa48f2fe94"


def test_r62_h17_preflight_freezes_all_files_and_pairs() -> None:
    report = payload()
...
def test_r62_h17_preflight_matches_exact_live_environment() -> None:
    report = payload()
...
def test_r62_h17_preflight_recorded_fresh_suite_absence() -> None:
