...
OVERLAY = (
    ROOT / "05_project/configs/experiments/v2_2_h17_candidate_r68.json"
)
WRAPPER = (
    ROOT / "05_project/scripts/"
    "run_protocol_v2_2_r68_h17_candidate_smoke.py"
)
RUNNER = ROOT / "05_project/scripts/run_protocol_v2_gate_f.py"
R67_STOP_REPORT = (
    ROOT / "reports/protocol_v2_2_r67_h17_candidate_stopped.json"
)
R68_LOCAL_REPORT = (
    ROOT / "reports/protocol_v2_2_r68_local_validation.json"
)
...
def test_r68_candidate_preserves_frozen_experiment_controls() -> None:
    wrapper = load_wrapper("r68_candidate_controls")
...
def test_r68_candidate_freeze_hashes_match_exact_source_commit() -> None:
    wrapper = load_wrapper("r68_candidate_freeze")
    manifest = json.loads(
        wrapper.build_candidate_manifest().read_text(encoding="utf-8")
    )
...
def test_r68_historical_static_validation_rejects_future_head_drift() -> None:
    wrapper = load_wrapper("r68_candidate_static_wrapper")
    runner = load_module(RUNNER, "r68_candidate_static_runner")
    manifest = json.loads(
        wrapper.build_candidate_manifest().read_text(encoding="utf-8")
    )
    with pytest.raises(
...
def test_r68_candidate_prerequisites_are_byte_exact() -> None:
    overlay = json.loads(OVERLAY.read_text(encoding="utf-8"))
    assert sha256(R67_STOP_REPORT.read_bytes()).hexdigest() == (
        overlay["prerequisite_r67_stop_report"]["sha256"]
    )
    stopped = json.loads(R67_STOP_REPORT.read_text(encoding="utf-8"))
    ...
    assert sha256(R68_LOCAL_REPORT.read_bytes()).hexdigest() == (
        overlay["prerequisite_r68_local_validation"]["sha256"]
    )
    local = json.loads(R68_LOCAL_REPORT.read_text(encoding="utf-8"))
    assert local["source_commit"] == (
        "41f6e39708de59c48c0211eb724f9391edd1a2fe"
    )
...
def test_r68_candidate_wrapper_rejects_every_other_live_scope(
    argv: list[str],
) -> None:
    wrapper = load_wrapper("r68_candidate_invalid_invocation")
    with pytest.raises(RuntimeError):
        wrapper.validate_invocation(argv)


def test_r68_candidate_wrapper_allows_only_preflight_or_h17_m0() -> None:
    wrapper = load_wrapper("r68_candidate_valid_invocation")
...
def test_r68_candidate_wrapper_uses_non_formal_mode() -> None:
...
def test_r68_mechanism_binds_detail_taps_and_repair_priority() -> None:
    guard = (
    ...
    assert "TARGET_ROW_DETAIL_CONTROL_GUARD" in guard
    assert "target_row_detail_non_control_tap" in guard
    assert "target_row_non_control_tap_block_count" in guard
    assert "validate_active_target_detail_control" in controller
    assert "ACTIVE_DETAIL_PRIORITY_OVERRIDE" in controller
    assert "compact_validation_error" in controller


def test_r68_mechanism_sources_are_task_and_answer_agnostic() -> None:
    paths = [
        ROOT / "05_project/src/raven_m/controller/protocol_v2_guard.py",
        ROOT / "05_project/src/raven_m/controller/episode_controller.py",
