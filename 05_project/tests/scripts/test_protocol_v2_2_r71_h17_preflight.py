...
REPORT = (
    ROOT / "reports/protocol_v2_2_r71_h17_candidate_preflight.json"
)
REPORT_SHA256 = (
    "8233ef0c2ec8311499ea1c208d3d35fb072a11706b247778c5f280ecc3330397"
)
...
def test_r71_h17_preflight_is_byte_frozen_and_zero_call() -> None:
    report = payload()
...
def test_r71_h17_preflight_matches_exact_source_and_package() -> None:
    report = payload()
    assert report["source_commit"] == (
        "422f33c587c83153a3f99b79ce1f3523032f7e87"
    )
    assert report["source_tag"] == "protocol-v2-2-r71-local-candidate"
    assert report["execution_commit"] == (
        "20d79fa06e2ff50399ef852085b20407196d1de9"
    )
    assert report["package_tag"] == (
        "protocol-v2-2-r71-h17-candidate-package"
    )
    ...
    assert resolved == "20d79fa06e2ff50399ef852085b20407196d1de9"


def test_r71_h17_preflight_freezes_all_files_and_pairs() -> None:
    report = payload()
...
def test_r71_h17_preflight_matches_exact_live_environment() -> None:
    report = payload()
...
def test_r71_h17_preflight_recorded_fresh_suite_absence() -> None:
    report = payload()
