...
REPORT = (
    ROOT / "reports/protocol_v2_2_r65_h17_candidate_preflight.json"
)
REPORT_SHA256 = (
    "7a182c89ba9ff87c4713931d3bbfaac318b0f427fbf0b07a6d219bd46495cf5d"
)
...
def test_r65_h17_preflight_is_byte_frozen_and_zero_call() -> None:
    report = payload()
...
def test_r65_h17_preflight_matches_exact_source_and_package() -> None:
    report = payload()
    assert report["source_commit"] == (
        "0eaf8ae0e157392ac33907a3cd06a57d18e00e07"
    )
    assert report["source_tag"] == "protocol-v2-2-r65-local-candidate"
    assert report["execution_commit"] == (
        "6e2db153b074f4710f7718b83ba72e9ed69c3f51"
    )
    resolved = subprocess.check_output(
        [
            ...
            "-n",
            "1",
            "protocol-v2-2-r65-h17-candidate-package",
        ],
        cwd=ROOT,
        text=True,
    ).strip()
    assert resolved == "6e2db153b074f4710f7718b83ba72e9ed69c3f51"


def test_r65_h17_preflight_freezes_all_files_and_pairs() -> None:
    report = payload()
...
def test_r65_h17_preflight_matches_exact_live_environment() -> None:
    report = payload()
...
def test_r65_h17_preflight_recorded_fresh_suite_absence() -> None:
