...
OVERLAY = (
    ROOT / "05_project/configs/experiments/v2_2_h17_candidate_r67.json"
)
WRAPPER = (
    ROOT / "05_project/scripts/"
    "run_protocol_v2_2_r67_h17_candidate_smoke.py"
)
RUNNER = ROOT / "05_project/scripts/run_protocol_v2_gate_f.py"
R66_STOP_REPORT = (
    ROOT / "reports/protocol_v2_2_r66_h17_candidate_stopped.json"
)
R67_LOCAL_REPORT = (
    ROOT / "reports/protocol_v2_2_r67_local_validation.json"
)
...
def test_r67_candidate_preserves_frozen_experiment_controls() -> None:
    wrapper = load_wrapper("r67_candidate_controls")
...
def test_r67_candidate_freeze_hashes_match_exact_source_commit() -> None:
    wrapper = load_wrapper("r67_candidate_freeze")
    manifest = json.loads(
        wrapper.build_candidate_manifest().read_text(encoding="utf-8")
    )
...
def test_r67_historical_static_validation_rejects_future_head_drift() -> None:
    wrapper = load_wrapper("r67_candidate_static_wrapper")
    runner = load_module(RUNNER, "r67_candidate_static_runner")
    manifest = json.loads(
        wrapper.build_candidate_manifest().read_text(encoding="utf-8")
    )
    with pytest.raises(
...
def test_r67_candidate_prerequisites_are_byte_exact() -> None:
    overlay = json.loads(OVERLAY.read_text(encoding="utf-8"))
    assert sha256(R66_STOP_REPORT.read_bytes()).hexdigest() == (
        overlay["prerequisite_r66_stop_report"]["sha256"]
    )
    stopped = json.loads(R66_STOP_REPORT.read_text(encoding="utf-8"))
    assert stopped["decision"] == (
        "r66_explicit_field_guard_validated_but_active_detail_ledger_"
        "precedence_and_context_cap_failed"
    )
    assert stopped["immutability"]["suite_may_be_resumed"] is False
    assert sha256(R67_LOCAL_REPORT.read_bytes()).hexdigest() == (
        overlay["prerequisite_r67_local_validation"]["sha256"]
    )
    local = json.loads(R67_LOCAL_REPORT.read_text(encoding="utf-8"))
...
def test_r67_candidate_wrapper_rejects_every_other_live_scope(
    argv: list[str],
) -> None:
    wrapper = load_wrapper("r67_candidate_invalid_invocation")
    with pytest.raises(RuntimeError):
        wrapper.validate_invocation(argv)


def test_r67_candidate_wrapper_allows_only_preflight_or_h17_m0() -> None:
    wrapper = load_wrapper("r67_candidate_valid_invocation")
...
def test_r67_candidate_wrapper_uses_non_formal_mode() -> None:
...
def test_r67_mechanism_enforces_active_detail_precedence() -> None:
    guard = (
    ...
    assert "TARGET_ROW_LEDGER_SCOPE_GUARD" in guard
    assert "target_row_coordinate_used_off_list" in guard
    assert "target_row_off_list_coordinate_block_count" in guard
    assert "ACTIVE_DETAIL_PRECEDENCE" in controller
    assert "deferred_row_coordinates_executable" in controller
    assert "_target_row_executor_memory_context" in controller
    assert "TARGET_ROW_DETAIL_INSPECTION_REPAIR" in controller


def test_r67_mechanism_sources_are_task_and_answer_agnostic() -> None:
    paths = [
        ROOT / "05_project/src/raven_m/controller/protocol_v2_guard.py",
        ROOT / "05_project/src/raven_m/controller/episode_controller.py",
