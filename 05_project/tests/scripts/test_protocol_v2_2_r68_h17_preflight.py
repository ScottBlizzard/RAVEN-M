...
REPORT = (
    ROOT / "reports/protocol_v2_2_r68_h17_candidate_preflight.json"
)
REPORT_SHA256 = (
    "c7e0f0a5f333dab047b1ecab20c2385685760775db9caec6da3dda36f0ef9ffb"
)
...
def test_r68_h17_preflight_is_byte_frozen_and_zero_call() -> None:
    report = payload()
...
def test_r68_h17_preflight_matches_exact_source_and_package() -> None:
    report = payload()
    assert report["source_commit"] == (
        "41f6e39708de59c48c0211eb724f9391edd1a2fe"
    )
    assert report["source_tag"] == "protocol-v2-2-r68-local-candidate"
    assert report["execution_commit"] == (
        "dc3c199c82e2db8eaa5a9b55ef0e409080548876"
    )
    resolved = subprocess.check_output(
        [
            ...
            "-n",
            "1",
            "protocol-v2-2-r68-h17-candidate-package",
        ],
        cwd=ROOT,
        text=True,
    ).strip()
    assert resolved == "dc3c199c82e2db8eaa5a9b55ef0e409080548876"


def test_r68_h17_preflight_freezes_all_files_and_pairs() -> None:
    report = payload()
...
def test_r68_h17_preflight_matches_exact_live_environment() -> None:
    report = payload()
...
def test_r68_h17_preflight_recorded_fresh_suite_absence() -> None:
