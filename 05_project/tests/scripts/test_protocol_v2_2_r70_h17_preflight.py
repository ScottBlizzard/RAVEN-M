...
REPORT = (
    ROOT / "reports/protocol_v2_2_r70_h17_candidate_preflight.json"
)
REPORT_SHA256 = (
    "ed30f11dfd69af6d7e214bc30c7eefbfe8c305a9eddc5111a4c79cc65c929cff"
)
...
def test_r70_h17_preflight_is_byte_frozen_and_zero_call() -> None:
    report = payload()
...
def test_r70_h17_preflight_matches_exact_source_and_package() -> None:
    report = payload()
    assert report["source_commit"] == (
        "1ccd6290b4584478d0eda7e602584d149888968c"
    )
    assert report["source_tag"] == "protocol-v2-2-r70-local-candidate"
    assert report["execution_commit"] == (
        "d0a3263c3e1a4dfa7cce5a48c9f866639795001b"
    )
    assert report["package_tag"] == (
        "protocol-v2-2-r70-h17-candidate-package"
    )
    ...
    assert resolved == "d0a3263c3e1a4dfa7cce5a48c9f866639795001b"


def test_r70_h17_preflight_freezes_all_files_and_pairs() -> None:
    report = payload()
...
def test_r70_h17_preflight_matches_exact_live_environment() -> None:
    report = payload()
...
def test_r70_h17_preflight_recorded_fresh_suite_absence() -> None:
    report = payload()
