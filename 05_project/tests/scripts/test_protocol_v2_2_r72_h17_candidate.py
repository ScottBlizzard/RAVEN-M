...
OVERLAY = (
    ROOT / "05_project/configs/experiments/v2_2_h17_candidate_r72.json"
)
WRAPPER = (
    ROOT / "05_project/scripts/"
    "run_protocol_v2_2_r72_h17_candidate_smoke.py"
)
RUNNER = ROOT / "05_project/scripts/run_protocol_v2_gate_f.py"
R71_STOP_REPORT = (
    ROOT / "reports/protocol_v2_2_r71_h17_candidate_stopped.json"
)
R72_LOCAL_REPORT = (
    ROOT / "reports/protocol_v2_2_r72_local_validation.json"
)
...
def test_r72_candidate_preserves_frozen_experiment_controls() -> None:
    wrapper = load_wrapper("r72_candidate_controls")
...
def test_r72_candidate_freeze_hashes_match_exact_source_commit() -> None:
    wrapper = load_wrapper("r72_candidate_freeze")
    manifest = json.loads(
        wrapper.build_candidate_manifest().read_text(encoding="utf-8")
    )
...
def test_r72_historical_static_validation_rejects_future_head_drift() -> None:
    wrapper = load_wrapper("r72_candidate_static_wrapper")
    runner = load_module(RUNNER, "r72_candidate_static_runner")
    manifest = json.loads(
        wrapper.build_candidate_manifest().read_text(encoding="utf-8")
    )
    with pytest.raises(
...
def test_r72_candidate_prerequisites_are_byte_exact() -> None:
    overlay = json.loads(OVERLAY.read_text(encoding="utf-8"))
    assert sha256(R71_STOP_REPORT.read_bytes()).hexdigest() == (
        overlay["prerequisite_r71_stop_report"]["sha256"]
    )
    stopped = json.loads(R71_STOP_REPORT.read_text(encoding="utf-8"))
    ...
    assert sha256(R72_LOCAL_REPORT.read_bytes()).hexdigest() == (
        overlay["prerequisite_r72_local_validation"]["sha256"]
    )
    local = json.loads(R72_LOCAL_REPORT.read_text(encoding="utf-8"))
...
def test_r72_candidate_wrapper_rejects_every_other_live_scope(
    argv: list[str],
) -> None:
    wrapper = load_wrapper("r72_candidate_invalid_invocation")
    with pytest.raises(RuntimeError):
        wrapper.validate_invocation(argv)


def test_r72_candidate_wrapper_allows_only_preflight_or_h17_m0() -> None:
    wrapper = load_wrapper("r72_candidate_valid_invocation")
...
def test_r72_candidate_wrapper_uses_non_formal_mode() -> None:
...
def test_r72_runner_wires_native_provider_only_for_v2_2() -> None:
    source = RUNNER.read_text(encoding="utf-8")
    assert "current_native_popup_menu_ui_elements" in source
    assert "native_popup_menu_provider=(" in source
    assert 'if manifest["protocol"] == PROTOCOL_V2_2' in source
    assert "else None" in source


def test_r72_native_route_is_narrow_and_fail_closed() -> None:
    adapter = (
    ...
    assert '"native_uiautomator_popup_row"' in adapter
    assert '"android.widget.ListView"' in adapter
    assert '"android.widget.LinearLayout"' in adapter
    assert "return candidates[0] if len(candidates) == 1 else []" in adapter
    assert "requested_field_ui_elements" in controller
    assert '"status": "failed_closed"' in controller
    assert "native_inspection_candidate_count" in guard
    assert "OCR" not in adapter


def test_r72_mechanism_sources_are_task_and_answer_agnostic() -> None:
    paths = [
        ROOT / "05_project/src/raven_m/controller/protocol_v2_guard.py",
        ROOT / "05_project/src/raven_m/controller/episode_controller.py",
        ROOT / "05_project/src/raven_m/env/androidworld_adapter.py",
