"""Preflight or run the one authorized non-scored r73 H17/M0 smoke."""
...
OVERLAY = (
    PROJECT_ROOT / "configs/experiments/v2_2_h17_candidate_r73.json"
)
GENERATED_MANIFEST = (
    REPOSITORY_ROOT
    / "runs/protocol_v2_2_development/manifests/"
    "v2_2_h17_candidate_r73.generated.json"
)
SOURCE_COMMIT = "0bb072f0c38654864e0521c53b572e5aaef6641e"
SOURCE_TAG = "protocol-v2-2-r73-local-candidate"
PARENT_GATE_E_COMMIT = "24ddb7a34c0e873218cbac6b081d7d24ecd7d61e"
R72_STOP_COMMIT = "b10358f6fec609c10a6479ce3207f5b2be882dc3"
R72_STOP_TAG = "protocol-v2-2-r72-h17-stopped"
...
def build_candidate_manifest() -> Path:
    """Build the r73 candidate from frozen r56 experimental controls."""
    overlay = json.loads(OVERLAY.read_text(encoding="utf-8"))
    if _git_output("rev-list", "-n", "1", SOURCE_TAG) != SOURCE_COMMIT:
        raise RuntimeError("r73 source tag drifted.")
    if _git_output("rev-list", "-n", "1", R72_STOP_TAG) != R72_STOP_COMMIT:
        raise RuntimeError("r72 stop tag drifted.")
    stopped = _verify_json_prerequisite(
        overlay["prerequisite_r72_stop_report"]
    )
    if (
        ...
        raise RuntimeError("r72 stop prerequisite is invalid.")
    local = _verify_json_prerequisite(
        overlay["prerequisite_r73_local_validation"]
    )
    if (
        local.get("decision")
        ...
        raise RuntimeError("r73 local-validation prerequisite is invalid.")

    base_path = REPOSITORY_ROOT / overlay["base_manifest"]
    manifest = json.loads(base_path.read_text(encoding="utf-8"))
    manifest.update(
        {
            "schema_version": "protocol_v2_2_h17_candidate_r73.v1",
            ...
            "prerequisite_r72_stop_report": overlay[
                "prerequisite_r72_stop_report"
            ],
            "prerequisite_r73_local_validation": overlay[
                "prerequisite_r73_local_validation"
            ],
        }
    )
    records_by_path = {
    ...
    if not overrides.keys() <= records_by_path.keys():
        raise RuntimeError("r73 freeze override is absent from r56.")
...
def validate_invocation(argv: list[str]) -> None:
    ...
    if "--batch" in argv or any(
        ...
        raise RuntimeError("r73 candidate wrapper forbids formal batches.")
    ...
    if "--preflight-only" in argv:
        if smoke_values:
            raise RuntimeError("r73 preflight cannot also launch a smoke.")
        return
    if smoke_values != [str(AUTHORIZED_SEQUENCE)]:
        raise RuntimeError(
            "r73 candidate permits only H17/M0 development sequence 2."
        )
