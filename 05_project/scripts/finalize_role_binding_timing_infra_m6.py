"""Independent terminal finalizer and manifest writer for INFRA-M6."""
...
from raven_m.role_binding_timing.infra_m6_terminal import finalize_completion  # noqa: E402
...
def build_manifest(output_root: Path) -> dict[str, Any]:
    ...
    value = {"schema_version": "role_binding_timing.infra_m6.manifest.v1", "artifacts": artifacts}
...
def main() -> int:
    ...
    final = finalize_completion(
        ...
        run_id=args.run_id,
        status=args.status,
        rich_completion=load_optional(output_root / "terminal_input.json"),
    )
    ...
    atomic_write_json(output_root / "terminal_validation.json", {
        "schema_version": "role_binding_timing.infra_m6.terminal_validation.v1",
        "passed": not errors,
        "errors": errors,
