"""Preflight or run the one authorized non-scored r63 H17/M0 smoke."""
...
OVERLAY = (
    PROJECT_ROOT / "configs/experiments/v2_2_h17_candidate_r63.json"
)
GENERATED_MANIFEST = (
    REPOSITORY_ROOT
    / "runs/protocol_v2_2_development/manifests/"
    "v2_2_h17_candidate_r63.generated.json"
)
SOURCE_COMMIT = "fe16e7dac9c383b400ca7ab7e4e760e29166ed5a"
SOURCE_TAG = "protocol-v2-2-r63-local-candidate"
PARENT_GATE_E_COMMIT = "24ddb7a34c0e873218cbac6b081d7d24ecd7d61e"
R62_STOP_COMMIT = "fe9c1b68cb7247acd60245f571c4204cb4056ea6"
R62_STOP_TAG = "protocol-v2-2-r62-h17-stopped"
...
def build_candidate_manifest() -> Path:
    """Build the r63 candidate from frozen r56 experimental controls."""
    overlay = json.loads(OVERLAY.read_text(encoding="utf-8"))
    if _git_output("rev-list", "-n", "1", SOURCE_TAG) != SOURCE_COMMIT:
        raise RuntimeError("r63 source tag drifted.")
    if _git_output("rev-list", "-n", "1", R62_STOP_TAG) != R62_STOP_COMMIT:
        raise RuntimeError("r62 stop tag drifted.")
    stopped = _verify_json_prerequisite(
        overlay["prerequisite_r62_stop_report"]
    )
    if (
        ...
        raise RuntimeError("r62 stop prerequisite is invalid.")
    local = _verify_json_prerequisite(
        overlay["prerequisite_r63_local_validation"]
    )
    if (
        local.get("decision")
        ...
        raise RuntimeError("r63 local-validation prerequisite is invalid.")

    base_path = REPOSITORY_ROOT / overlay["base_manifest"]
    manifest = json.loads(base_path.read_text(encoding="utf-8"))
    manifest.update(
        {
            "schema_version": "protocol_v2_2_h17_candidate_r63.v1",
            ...
            "prerequisite_r62_stop_report": overlay[
                "prerequisite_r62_stop_report"
            ],
            "prerequisite_r63_local_validation": overlay[
                "prerequisite_r63_local_validation"
            ],
        }
    )
    records_by_path = {
    ...
    if not overrides.keys() <= records_by_path.keys():
        raise RuntimeError("r63 freeze override is absent from r56.")
...
def validate_invocation(argv: list[str]) -> None:
    ...
    if "--batch" in argv or any(
        ...
        raise RuntimeError("r63 candidate wrapper forbids formal batches.")
    ...
    if "--preflight-only" in argv:
        if smoke_values:
            raise RuntimeError("r63 preflight cannot also launch a smoke.")
        return
    if smoke_values != [str(AUTHORIZED_SEQUENCE)]:
        raise RuntimeError(
            "r63 candidate permits only H17/M0 development sequence 2."
        )
