"""Preflight or run the one authorized non-scored r61 H17/M0 smoke."""
...
OVERLAY = (
    PROJECT_ROOT / "configs/experiments/v2_2_h17_candidate_r61.json"
)
GENERATED_MANIFEST = (
    REPOSITORY_ROOT
    / "runs/protocol_v2_2_development/manifests/"
    "v2_2_h17_candidate_r61.generated.json"
)
SOURCE_COMMIT = "baa398babf9707f32eb94d1287e5bc2d728a84bb"
SOURCE_TAG = "protocol-v2-2-r61-local-candidate"
PARENT_GATE_E_COMMIT = "24ddb7a34c0e873218cbac6b081d7d24ecd7d61e"
R60_STOP_COMMIT = "85dbf649a8683a46af01a8fdf21de7786ae025a2"
R60_STOP_TAG = "protocol-v2-2-gate-f-r60-stopped"
...
def build_candidate_manifest() -> Path:
    """Build the r61 candidate from frozen r56 experimental controls."""
    overlay = json.loads(OVERLAY.read_text(encoding="utf-8"))
    if _git_output("rev-list", "-n", "1", SOURCE_TAG) != SOURCE_COMMIT:
        raise RuntimeError("r61 source tag drifted.")
    if _git_output("rev-list", "-n", "1", R60_STOP_TAG) != R60_STOP_COMMIT:
        raise RuntimeError("r60 formal stop tag drifted.")
    stopped = _verify_json_prerequisite(
        overlay["prerequisite_r60_stop_report"]
    )
    if (
        stopped.get("decision")
        != "formal_gate_f_stopped_failed_at_2_of_12"
        or stopped.get("immutability", {}).get("suite_may_be_resumed")
        is not False
    ):
        raise RuntimeError("r60 formal-stop prerequisite is invalid.")
    local = _verify_json_prerequisite(
        overlay["prerequisite_r61_local_validation"]
    )
    if (
        local.get("decision")
        ...
        raise RuntimeError("r61 local-validation prerequisite is invalid.")

    base_path = REPOSITORY_ROOT / overlay["base_manifest"]
    manifest = json.loads(base_path.read_text(encoding="utf-8"))
    manifest.update(
        {
            "schema_version": "protocol_v2_2_h17_candidate_r61.v1",
            ...
            "prerequisite_r60_stop_report": overlay[
                "prerequisite_r60_stop_report"
            ],
            "prerequisite_r61_local_validation": overlay[
                "prerequisite_r61_local_validation"
            ],
        }
    )
    records_by_path = {
    ...
    if not overrides.keys() <= records_by_path.keys():
        raise RuntimeError("r61 freeze override is absent from r56.")
...
def validate_invocation(argv: list[str]) -> None:
    ...
    if "--batch" in argv or any(
        ...
        raise RuntimeError("r61 candidate wrapper forbids formal batches.")
    ...
    if "--preflight-only" in argv:
        if smoke_values:
            raise RuntimeError("r61 preflight cannot also launch a smoke.")
        return
    if smoke_values != [str(AUTHORIZED_SEQUENCE)]:
        raise RuntimeError(
            "r61 candidate permits only H17/M0 development sequence 2."
        )
