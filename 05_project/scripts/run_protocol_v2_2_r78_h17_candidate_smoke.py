"""Preflight or run the one authorized non-scored r78 H17/M0 smoke."""
...
OVERLAY = (
    PROJECT_ROOT / "configs/experiments/v2_2_h17_candidate_r78.json"
)
GENERATED_MANIFEST = (
    REPOSITORY_ROOT
    / "runs/protocol_v2_2_development/manifests/"
    "v2_2_h17_candidate_r78.generated.json"
)
SOURCE_COMMIT = "69ab866b08f3af13f304bdc99f2af0a985cff0cf"
SOURCE_TAG = "protocol-v2-2-r78-local-candidate"
PARENT_GATE_E_COMMIT = "24ddb7a34c0e873218cbac6b081d7d24ecd7d61e"
R77_STOP_COMMIT = "c68931e2e4b45c5eecdef6261c1dbcb65e0a7cae"
R77_STOP_TAG = "protocol-v2-2-r77-h17-stopped"
...
def build_candidate_manifest() -> Path:
    """Build the r78 candidate from frozen r56 experimental controls."""
    overlay = json.loads(OVERLAY.read_text(encoding="utf-8"))
    if _git_output("rev-list", "-n", "1", SOURCE_TAG) != SOURCE_COMMIT:
        raise RuntimeError("r78 source tag drifted.")
    if _git_output("rev-list", "-n", "1", R77_STOP_TAG) != R77_STOP_COMMIT:
        raise RuntimeError("r77 stop tag drifted.")
    stopped = _verify_json_prerequisite(
        overlay["prerequisite_r77_stop_report"]
    )
    if (
        stopped.get("decision")
        != "r77_solved_h17_and_validated_same_episode_routed_evidence_"
        "but_protocol_stopped_on_executor_context_cap"
        or stopped.get("immutability", {}).get("suite_may_be_resumed")
        is not False
    ):
        raise RuntimeError("r77 stop prerequisite is invalid.")
    local = _verify_json_prerequisite(
        overlay["prerequisite_r78_local_validation"]
    )
    if (
        local.get("decision")
        ...
        raise RuntimeError("r78 local-validation prerequisite is invalid.")

    base_path = REPOSITORY_ROOT / overlay["base_manifest"]
    manifest = json.loads(base_path.read_text(encoding="utf-8"))
    manifest.update(
        {
            "schema_version": "protocol_v2_2_h17_candidate_r78.v1",
            ...
            "prerequisite_r77_stop_report": overlay[
                "prerequisite_r77_stop_report"
            ],
            "prerequisite_r78_local_validation": overlay[
                "prerequisite_r78_local_validation"
            ],
        }
    )
    records_by_path = {
    ...
    if not overrides.keys() <= records_by_path.keys():
        raise RuntimeError("r78 freeze override is absent from r56.")
...
def validate_invocation(argv: list[str]) -> None:
    ...
    if "--batch" in argv or any(
        ...
        raise RuntimeError("r78 candidate wrapper forbids formal batches.")
    ...
    if "--preflight-only" in argv:
        if smoke_values:
            raise RuntimeError("r78 preflight cannot also launch a smoke.")
        return
    if smoke_values != [str(AUTHORIZED_SEQUENCE)]:
        raise RuntimeError(
            "r78 candidate permits only H17/M0 development sequence 2."
        )
