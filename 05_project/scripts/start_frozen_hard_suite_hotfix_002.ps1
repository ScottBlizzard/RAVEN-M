...
$pidFile = Join-Path $suiteRoot "hotfix_002_runner.pid"
...
if (Test-Path -LiteralPath $pidFile) {
    ...
    if ($existing) {
        Write-Host "Hotfix-002 Hard suite already running: PID $oldPid"
        exit 0
    }
}

$arguments = @(
    (Join-Path $PSScriptRoot "run_frozen_hard_suite_hotfix_002.py"),
    "--adb-path", $adb,
    "--phase", $Phase,
    "--suite-id", $SuiteId,
    "--max-model-recovery-seconds", $MaxModelRecoverySeconds
)
$stdout = Join-Path $logRoot "hotfix_002.stdout.log"
$stderr = Join-Path $logRoot "hotfix_002.stderr.log"
$process = Start-Process -FilePath $python -ArgumentList $arguments `
    -WorkingDirectory $repositoryRoot -WindowStyle Hidden -PassThru `
    -RedirectStandardOutput $stdout -RedirectStandardError $stderr
Set-Content -LiteralPath $pidFile -Value $process.Id -Encoding ascii
Write-Host "Started hotfix-002 Hard $Phase suite with PID $($process.Id)."
