"""Preflight or run the one authorized non-scored r67 H17/M0 smoke."""
...
OVERLAY = (
    PROJECT_ROOT / "configs/experiments/v2_2_h17_candidate_r67.json"
)
GENERATED_MANIFEST = (
    REPOSITORY_ROOT
    / "runs/protocol_v2_2_development/manifests/"
    "v2_2_h17_candidate_r67.generated.json"
)
SOURCE_COMMIT = "f4361f464e5beb43ff2fd36d908cc61ad9824815"
SOURCE_TAG = "protocol-v2-2-r67-local-candidate"
PARENT_GATE_E_COMMIT = "24ddb7a34c0e873218cbac6b081d7d24ecd7d61e"
R66_STOP_COMMIT = "d285449aaf42886f9c1f2001de975f51b5d97b6e"
R66_STOP_TAG = "protocol-v2-2-r66-h17-stopped"
...
def build_candidate_manifest() -> Path:
    """Build the r67 candidate from frozen r56 experimental controls."""
    overlay = json.loads(OVERLAY.read_text(encoding="utf-8"))
    if _git_output("rev-list", "-n", "1", SOURCE_TAG) != SOURCE_COMMIT:
        raise RuntimeError("r67 source tag drifted.")
    if _git_output("rev-list", "-n", "1", R66_STOP_TAG) != R66_STOP_COMMIT:
        raise RuntimeError("r66 stop tag drifted.")
    stopped = _verify_json_prerequisite(
        overlay["prerequisite_r66_stop_report"]
    )
    if (
        ...
        raise RuntimeError("r66 stop prerequisite is invalid.")
    local = _verify_json_prerequisite(
        overlay["prerequisite_r67_local_validation"]
    )
    if (
        local.get("decision")
        ...
        raise RuntimeError("r67 local-validation prerequisite is invalid.")

    base_path = REPOSITORY_ROOT / overlay["base_manifest"]
    manifest = json.loads(base_path.read_text(encoding="utf-8"))
    manifest.update(
        {
            "schema_version": "protocol_v2_2_h17_candidate_r67.v1",
            ...
            "prerequisite_r66_stop_report": overlay[
                "prerequisite_r66_stop_report"
            ],
            "prerequisite_r67_local_validation": overlay[
                "prerequisite_r67_local_validation"
            ],
        }
    )
    records_by_path = {
    ...
    if not overrides.keys() <= records_by_path.keys():
        raise RuntimeError("r67 freeze override is absent from r56.")
...
def validate_invocation(argv: list[str]) -> None:
    ...
    if "--batch" in argv or any(
        ...
        raise RuntimeError("r67 candidate wrapper forbids formal batches.")
    ...
    if "--preflight-only" in argv:
        if smoke_values:
            raise RuntimeError("r67 preflight cannot also launch a smoke.")
        return
    if smoke_values != [str(AUTHORIZED_SEQUENCE)]:
        raise RuntimeError(
            "r67 candidate permits only H17/M0 development sequence 2."
        )
