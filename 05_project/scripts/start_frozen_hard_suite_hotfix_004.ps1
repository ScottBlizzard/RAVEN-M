...
$pidFile = Join-Path $suiteRoot "hotfix_004_runner.pid"
...
if (Test-Path -LiteralPath $pidFile) {
    ...
    if ($existing) {
        Write-Host "Hotfix-004 Hard suite already running: PID $oldPid"
        exit 0
    }
}

$arguments = @(
    (Join-Path $PSScriptRoot "run_frozen_hard_suite_hotfix_004.py"),
    "--adb-path", $adb,
    "--phase", $Phase,
    "--suite-id", $SuiteId,
    "--max-model-recovery-seconds", $MaxModelRecoverySeconds
)
$stdout = Join-Path $logRoot "hotfix_004.stdout.log"
$stderr = Join-Path $logRoot "hotfix_004.stderr.log"
$process = Start-Process -FilePath $python -ArgumentList $arguments `
    -WorkingDirectory $repositoryRoot -WindowStyle Hidden -PassThru `
    -RedirectStandardOutput $stdout -RedirectStandardError $stderr
Set-Content -LiteralPath $pidFile -Value $process.Id -Encoding ascii
Write-Host "Started hotfix-004 Hard $Phase suite with PID $($process.Id)."
