"""Run zero-device-mutation offline gates for INFRA-M4."""
...
OUTPUT_ROOT = PROJECT_ROOT / "artifacts/role_binding_timing/infra_m4_offline_gates"
CONFIG_PATH = PROJECT_ROOT / "configs/role_binding_timing/infra_m4_terminal_accounting_hardening.json"
...
def load_m1() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("m1_for_m4_gate", path)
...
def main() -> int:
    if OUTPUT_ROOT.exists():
        raise RuntimeError("M4_OFFLINE_GATE_ROOT_NOT_FRESH")
    OUTPUT_ROOT.mkdir(parents=True)
    (OUTPUT_ROOT / ".gitattributes").write_text("*.bin binary\n", encoding="ascii")
    config = json.loads(CONFIG_PATH.read_text(encoding="utf-8"))
    project = PROJECT_ROOT
    gates = [
        run_gate("compile", [str(PYTHON), "-m", "py_compile", "src/raven_m/role_binding_timing/infra_m4_terminal_accounting.py", "scripts/finalize_role_binding_timing_infra_m4.py", "scripts/run_role_binding_timing_infra_m4.py", "scripts/run_role_binding_timing_infra_m4_offline_gates.py"], project),
        run_gate("focused", [str(PYTHON), "-m", "pytest", "tests/role_binding_timing/test_infra_m4_terminal_accounting.py", "-q", "--color=no"], project),
    ...
    sample = {
        ...
        "first_broken_edge": "OFFLINE", "last_completed_phase": "launch",
    ...
    corruptions = []
    for key, value in (("generation_calls", 1), ("held_out_captures", 1), ("protected_wip_unchanged", False), ("journal_terminal_event_present", False)):
        corrupt = dict(sample)
        corrupt[key] = value
        corruptions.append({"key": key, "detected": bool(list(validator.iter_errors(corrupt)))})

    protected = {name: sha256((REPOSITORY_ROOT / name).read_bytes()).hexdigest() for name in config["protected_wip"]}
    runner_source = (PROJECT_ROOT / "scripts/run_role_binding_timing_infra_m4.py").read_text(encoding="utf-8")
    finalizer_source = (PROJECT_ROOT / "scripts/finalize_role_binding_timing_infra_m4.py").read_text(encoding="utf-8")
    accounting_source = (PROJECT_ROOT / "src/raven_m/role_binding_timing/infra_m4_terminal_accounting.py").read_text(encoding="utf-8")
    protocol_source = (REPOSITORY_ROOT / "04_protocols/role_binding_timing/INFRA_M4_TERMINAL_ACCOUNTING_HARDENING_V1.md").read_text(encoding="utf-8")
    joined = runner_source + "\n" + finalizer_source + "\n" + accounting_source + "\n" + protocol_source + "\n" + CONFIG_PATH.read_text(encoding="utf-8")
    source_gate = {
        "no_foreign_terminal_writer": "M1.write_json_atomic" not in runner_source + finalizer_source + accounting_source,
        "local_atomic_writer": "def atomic_write_json" in accounting_source,
        "write_once_first_edge": "first_broken_edge.json" in accounting_source and "replace=False" in accounting_source,
        "independent_finalizer": "invoke_independent_finalizer" in runner_source and "finalize_completion" in finalizer_source,
        "all_phases_journaled": all(f'"{phase}"' in runner_source and phase in config["terminal_accounting"]["phase_order"] for phase in config["terminal_accounting"]["phase_order"]),
        "external_temp_present": "create_live_root" in runner_source and "seal_live_logs" in runner_source,
        "no_5037_adb_command": "adb_prefix(config, 5037" not in runner_source,
        "zero_generation_boundary": '"generation_calls_authorized": 0' in joined,
        "no_prior_frozen_log_path": "infra_m1_maintenance_burnin/maintenance/start/emulator.stdout.bin" not in runner_source,
    }
    ...
    if any(snapshot["listeners"][str(port)] for port in (5037, 5038, 5554, 5555, 8554)):
        runtime_issues.append("LISTENER_PRESENT")
    if snapshot.get("adb_pid") or snapshot.get("launcher_pid") or snapshot.get("qemu_pid"):
        runtime_issues.append("OWNED_PROCESS_PRESENT")
    if snapshot["excluded_runtime_pids"] != config["pre_cleanup_identity"]["excluded_runtime_pids"]:
        runtime_issues.append("EXCLUDED_PID_DRIFT")
    if (REPOSITORY_ROOT / config["output_root"]).exists():
        runtime_issues.append("OUTPUT_ROOT_EXISTS")
    ...
    if temp_residue:
        runtime_issues.append("TEMP_RESIDUE")
    ...
    result = {
        "schema_version": "role_binding_timing.infra_m4.offline_gates.v1",
        ...
        "schema_gate": {"passed": not schema_errors and all(item["detected"] for item in corruptions), "errors": schema_errors, "corruptions": corruptions},
    ...
    result["overall_pass"] = all((
        ...
        result["binary_hashes_pass"], legacy_restore["passed"],
    ))
