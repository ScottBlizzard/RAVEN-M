"""Run the frozen B2.9 zero-model Settings AndroidEnv sidecar diagnosis."""
...
from raven_m.role_binding_timing.command_output_v0_2_9 import (  # noqa: E402
...
FREEZE_TAG = "role-binding-timing-b2.9-sidecar-diagnosis-freeze-20260804"
PROTECTED_WIP = {
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_ROOT / "configs/role_binding_timing/phase_b2_9_androidenv_sidecar_diagnosis.json",
    )
    ...
    lock_path = PROJECT_ROOT / "configs/role_binding_timing/phase_b2_9_androidenv_sidecar_diagnosis.lock.json"
    ...
    try:
        ...
        for service_name in ("package", "window", "activity"):
            ...
            if not service_check["passed"]:
                raise RuntimeError(f"FRAMEWORK_PREFLIGHT:{service_name}:{service_check['issues']}")
    ...
    if protected_after != PROTECTED_WIP:
        qualification_passed = False
    completion = {
        "schema_version": "role_binding_timing.phase_b2_9.diagnosis_completion.v0.2.9",
