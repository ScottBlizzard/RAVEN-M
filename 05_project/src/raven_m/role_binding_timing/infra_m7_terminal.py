"""Independent exactly-once terminal completion for INFRA-M7."""
...
def minimal_completion(*, run_id: str, status: str, first_edge: str | None) -> dict[str, Any]:
    return {
        "schema_version": "role_binding_timing.infra_m7.completion.v1",
        "terminal_mode": "minimal_fallback", "run_id": run_id, "status": status,
        "first_broken_edge": first_edge, "completed_at": utc_now(),
        ...
        "claim_evidence": {
            "runner_adb_authority_qualified": False, "display_quorum_qualified": False,
            "process_identity_qualified": False, "exclusive_5038_registration": False,
            "burn_in_qualified": False, "a11y_tested": False, "a11y_qualified": False,
...
def finalize_completion(
    *, output_root: Path, journal: Any, run_id: str, status: str,
    ...
    mode, rich_error = "minimal_fallback", None
    if rich_completion is not None:
        try:
            ...
            claims["runner_adb_authority_qualified"] = status == "PASS_12_OF_12_DEV"
            rich.update({
                "schema_version": "role_binding_timing.infra_m7.completion.v1",
    ...
    final.update({"terminal_mode": mode, "first_broken_edge": journal.first_edge(),
    ...
    atomic_write_json(output_root / "terminal_writer_receipt.json", {
        "schema_version": "role_binding_timing.infra_m7.terminal_receipt.v1",
        "terminal_mode": mode, "canonical_path": str(canonical),
        "canonical_sha256": sha256(canonical.read_bytes()).hexdigest(),
        "first_broken_edge": journal.first_edge(), "journal_entry_count": len(journal.read_entries()),
